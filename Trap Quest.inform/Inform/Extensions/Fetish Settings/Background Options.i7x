...
"Background (only one): D&D Nerd (you start out with slightly higher base ingelligence, slightly lower base strength, some starting magic power and a random spell) - 5 points ([if background-magic is 0]not [otherwise][bold type][end if]chosen[roman type])"	--	--	magic background toggle rule
