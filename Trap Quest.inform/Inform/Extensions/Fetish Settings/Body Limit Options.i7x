...
"Futanari content (Can biologically female people have penises? If disabled, they'll have strapons instead) -[if futanari fetish > 0] [bold type]enabled[roman type][otherwise if choice in row 68 of the Table of Player Options is 0 or the player is not a top donator] disabled[end if][if futanari fetish is 1] (WITH balls)[otherwise if futanari fetish is 2] (WITHOUT balls)[end if][if choice in row 68 of the Table of Player Options > 0 and lady fetish is 2 and the player is a top donator] (player only, since all NPCs are masculine)[end if]"	--	--	futanari fetish rule
