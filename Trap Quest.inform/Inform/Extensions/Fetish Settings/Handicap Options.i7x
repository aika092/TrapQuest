...
"RANDOMISE FLEXIBLE CHOICES (this randomises choices on 'fetish options' page too)"	--	--	randomise choices rule
"NIGHTMARE MODE: Enable all your flexible choices! (this enables choices on 'fetish options' page too)"	--	--	nightmare mode rule
...
"Starting body shape ([if starting body shape is 0 and the player is male]normal[otherwise if starting body shape is 0]androgynous[otherwise if starting body shape is 1 and the player is male]androgynous[otherwise if starting body shape is 1]normal[otherwise if the player is male]THICC[otherwise]curvy[end if] [if the remainder after dividing choice in row 62 of the Table of Player Options by 2 is 0]always[otherwise]but can be randomised[end if] - [starting body shape * 2] points)"	--	--	starting body shape toggle rule
...
The sex fainting random rule is listed in the random mode rules.
...
The ungape random rule is listed in the random mode rules.
...
The inventory handicap random rule is listed in the random mode rules.
...
To decide which number is realisticArms:
	if the player is in a predicament room and (the player is not in Predicament01 or (current-predicament is vibe-photo-predicament and the predicament-progression of vibe-photo-predicament > 0)) and the player is not in Predicament20, decide on 1;
	decide on 0.
