...
To compute cursed drinking (X - a squirt dildo):
	say "An extremely bitter taste and slimy consistency immediately lets you know it's not what you put in. The squirt dildo changed everything inside it into [semen]! [variable custom style]'One of these is cursed, but I don[']t know which...'[roman type][line break]";
	StomachSemenUp 1.
...
To compute cursed drinking (X - a baby's bottle):
	...
	otherwise if diaper lover >= 1 and incontinence < 5:
		say "you feel a weird twang internally, just behind your [if the player is male][ShortDesc of penis][otherwise][vagina][end if], where your bladder should be.[if the bimbo of the player < 8][one of][variable custom style]That can't be good...[roman type][line break][or][stopping][end if]";
...
To compute cursed drinking (X - a teapot):
	if diaper quest is 1:
		...
		otherwise:
			...
			DiaperAddictUp 1;
	otherwise:
		say "An extremely bitter taste and slimy consistency immediately lets you know it's not what you put in. The teapot changed everything inside it into [semen]! [if the Known corresponding to an Magic of the fill-type of X in the Table of Drinks is 0][one of][variable custom style]'Either the teapot or the drink colour is cursed, but I don[']t know which...'[roman type][or][stopping][end if][line break]";
		StomachSemenUp 1.
