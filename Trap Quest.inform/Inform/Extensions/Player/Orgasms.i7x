...
[!<RuinBodyPartTimesNumber>+

REQUIRES COMMENTING

+!]
To ruin (X - a body part) times (N - a number):
	let Z be N;
	while Z > 0:
		ruin X;
		decrease Z by 1.
...
+!]
To ruin (X - a body part):
	if X is asshole, AssRuin 1;
	if X is vagina, PussyRuin 1.
...
To anally orgasm shamefully:
	...
	otherwise:
		...
		say "[if the humiliation of the player < 20000][first custom style]What a shameful way to orgasm...[otherwise if the humiliation of the player < 30000][variable custom style]Cumming like that is so humiliating.  So why do I find myself just getting turned on even more?[otherwise][second custom style]I came just from my butthole... I'm so dirty![end if][roman type][line break]";
		orgasm;
	...
	if refactoryperiod > 0:
		if there is a camera trap in the location of the player and there is an off-stage shameful orgasm poster:
			...
			say "[one of][variable custom style][if the humiliation of the player < 20000]Oh shit.  I bet I didn't look very dignified right then.[otherwise]Now everyone will know that I came from my [asshole].[end if][or][if the bimbo of the player < 14][variable custom style]Again?![otherwise][second custom style]I bet that pic looks FILTHY![end if][stopping][roman type][line break]";
			set up P;
		humiliate 250;
...
To vaginally orgasm shamefully:
	...
	otherwise:
		...
		if interracial fetish is 1 and there is a dark skinned male monster penetrating vagina, say "[one of][if the sex addiction of the player < 6][first custom style]I can't believe I just came from a black [manly-penis]...[otherwise if the raw sex addiction of the player < 11][variable custom style]I just came all over a black [manly-penis]...  I didn't know it would feel so good.[otherwise][second custom style]Oh my god, black [manly-penis] is the best.  I want more![end if][or][if the sex addiction of the player < 6][first custom style]What is it about black [manly-penis] that makes me cum, even when I don't want to?![otherwise if the sex addiction of the player < 9][first custom style]If this keeps happening, I know I won't be able to stop myself from genuinely enjoying getting railed by big black [manly-penis]. [second custom style]I might be enjoying it already...[otherwise if the sex addiction of the player < 12][variable custom style]I only cum this hard when it's a black [manly-penis]![otherwise][second custom style]That felt so FUCKING good! I need even more black [manly-penis]![end if][stopping][roman type][line break]";
		otherwise say "[one of][if the sex addiction of the player < 8][first custom style]I can't believe I just came...[otherwise if the raw sex addiction of the player < 13][variable custom style]I just came...I didn't know it would feel so good.[otherwise][second custom style]That felt sooooo good...I want more![end if][or][if the sex addiction of the player < 8][first custom style]I have to find a way to stop orgasming like this...[otherwise if the sex addiction of the player < 13][first custom style]If this keeps happening, I know I'll get addicted. [second custom style]I might be already...[otherwise][second custom style]That felt so FUCKING good! More![end if][stopping][roman type][line break]";
...
To compute ejaculation:
	let P be a random bottom level pee protection clothing worn by the player;
	if P is clothing:
		cumsoak the size of penis on P;
	otherwise:
...
[!<TheGuardOrgasmCutsceneRule>+

REQUIRES COMMENTING

+!]
This is the guard orgasm cutscene rule:
	if there is a royal guard penetrating a fuckhole and image cutscenes is 1, display figure of guard cutscene 3.
The guard orgasm cutscene rule is listed last in the orgasm resolution rules.
...
This is the girls pee when they orgasm rule:
	if the player is female and the bladder of the player > 6 and player-urinating is 0:
...
This is the hentai orgasm resolution rule:
	if the milk volume of breasts > 4:[###maybe have this influenced by breast sensitivity]
		...
		say "[one of][variable custom style]Good grief! That was a most peculiar feeling![roman type][or][variable custom style]Wow, that was a strange coincidence! I hope.[roman type][or]Once again, you seem to squirt milk [i]as[/i] you climaxed. [variable custom style]Huh! I thought that only happened in Hentai fiction![roman type] I hope that's not going to happen [i]every[/i] time![or]Oh, man... again? You feel your face turn red.[variable custom style] I think my boobs have been turned into helplessly-squirting milk dispensers when I come![roman type] Surely, not?  Please?[or]Oh, no! [variable custom style]It's true: each time I come, my boobies squirt milky goodness![roman type] Worse, you can't help but privately thrill at the feel of the thin fluid squirting from your so-sensitive and swollen nips![or][variable custom style]Oooh, yesss! Squirt, squirt! So good! [roman type] Uh... you feel your face flush pink in shame at just how much you enjoyed that sensation![or][variable custom style]Here we go again - yum![roman type] So I ejaculate milk from my boobies now? So what? Is it really such a big deal?[roman type] You feel the warm milk running wetly down your swollen boobs.[or][variable custom style]I love my milky boobs.[roman type][stopping][roman type][line break]";
		humiliate 40;
...
This is the chastity cage orgasm resolution rule:
	if there is a worn chastity cage:
		if a random worn chastity cage is not cursed:
			say "You feel your [random worn chastity cage] become cursed again!";
...
Definition: a fuckhole (called F) is pushed over the edge:
	...
	let N be -1;
	if a random thing penetrating F is live, increase N by 1;
