...
Definition: a clothing (called C) is short-skirt-disallowed rather than short-skirt-allowed:
	if short skirts permanent fetish > 0:
		...
		if (the front-skirt-length of C > short skirts permanent fetish or the back-skirt-length of C > short skirts permanent fetish) and C is crotch-in-place and C is not crotch-unzipped and C is actually dense, decide yes;
	decide no.

short skirts permanent fetish is a number that varies. short skirts permanent fetish is 0. [When above 0, this is the highest length of skirt tolerated]
...
An all time based rule (this is the fighting against short skirts fetish rule):
	if the player is fighting against short skirts:
		if short skirts reminder <= 0:
			...
			if C is clothing, say "[bold type]You [one of][or]still [stopping]feel extremely uncomfortable in your [ShortDesc of C]! [one of]You can feel your body desperately wishing to be in [if C is unskirted and there is a worn skirted clothing]just a skirt[otherwise if C is unskirted]a skirt[otherwise if the front-skirt-length of C - 2 <= short skirts permanent fetish and the back-skirt-length of C - 2 <= short skirts permanent fetish]shorter skirt[otherwise]much shorter skirt[end if], and it's not only keeping you constantly distracted, but also making you feel much more stiff.[or][stopping][roman type][line break]";
