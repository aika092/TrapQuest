...
+!]
To decide which number is the soreness-influence of (C - a thing):
	decide on 0.
...
To decide which number is the stimulation of (M - a monster) on (F - a fuckhole):
	...
	if interracial fetish is 1:
		if M is dark skinned:
			increase S by 1;
			if the player is queen of spades, increase S by 1;
		otherwise if there is a monster penetrating a fuckhole:
			if the player is queen of spades, decrease S by 5;
	if S < 1, decide on 1;
	decide on S.
