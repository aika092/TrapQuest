...
Definition: a for deposit only tattoo (called A) is drawable:
	...
	decide yes.
Definition: a for deposit only tattoo (called T) is eligible:	
...
This is the deposit only tattoo blocks masturbation rule:
	if there is a worn for deposit only tattoo:
		if auto is 0, say "Your [printed name of random worn for deposit only tattoo] is somehow preventing you!";
		rule fails.
