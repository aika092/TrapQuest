...
Definition: a bird nest tattoo (called T) is drawable:
	...
	decide no.
Definition: a bird nest tattoo (called T) is eligible:
	if the noun is santa hat, decide yes;
	decide no.
To say tattoo-desc of (T - a bird nest tattoo):
...
A time based rule (this is the Christmas tattoo rule):
	if there is a worn bird nest tattoo and a random number between 1 and 100 is 1:
