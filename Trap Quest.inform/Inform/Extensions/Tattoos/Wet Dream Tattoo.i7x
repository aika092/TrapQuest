Wet Dream Tattoo by Tattoos begins here.

wet dream tattoo is a kind of chest tattoo.  Figure of wet dream tattoo is the file "tattoo20.png".  There is 1 wet dream tattoo.
Definition: a wet dream tattoo (called T) is drawable:
	if there is a worn chest tattoo, decide no;
	if diaper quest is 1, decide no;
	decide yes.
Definition: a wet dream tattoo (called T) is eligible:
	if the noun is clothing:
		if the semen-soak of the noun > 0, decide yes;
	decide no.
To say tattoo-desc of (T - a wet dream tattoo):
	if images visible is 1, display figure of wet dream tattoo;
	say "The words 'Wet Dream' are tattooed in large letters across your [BreastDesc].".

Wet Dream Tattoo ends here.
