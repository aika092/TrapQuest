...
To compute attack of (W - a notebook) at (M - a monster):
	...
	if M is notebook-studied:
		say "The [printed name of W] emits pink and red sparks as you thrust your hand at the [M], channelling the energy into a [if I < 3]small[otherwise if I < 6]big[otherwise if I < 9]huge[otherwise]massive[end if], [one of]fizzling[or]crackling[or]sizzling[at random], [one of]dick[or]cock[or]penis[at random]-shaped magic missile!";
	otherwise:
		say "The pages of the [printed name of W] glow as you thrust your hand at the [M], channelling the energy into a red, [one of]fizzling[or]crackling[or]sizzling[at random], [if I < 3]arrow-shaped[otherwise if I < 6]axe-shaped[otherwise]sword-shaped[end if] magic missile!".
...
To compute studying (N - a number) of (M - a monster):
	...
	if N is 0:[sex]
		if M is notebook-studied:
			say "A yellow bolt shoots out of the [printed name of G] disappears into the [M]'s chest. [big his of M] expression changes slightly, and somehow you know [he of M] looks more favourable on you now.";
			FavourUp M by 1;
		if a random number between 0 and the times-fucked of M < C:
			say "The [printed name of G] shivers, as if a draft were passing through its pages. Looks like it wants to know more about the [M]!";
		otherwise:
			say "The [printed name of G] falls open, filling several pages with detailed sketches of the [M]'s genitals, which slowly meld with the inscruable script that covers the rest of its pages. The book seems a lot stronger!";
	...
	otherwise:[kill]
		...
		otherwise:
			say "The [printed name of G] falls open, filling a few with detailed sketches of the [M], which slowly meld with the inscruable scrip that covers the rest of its pages. The book seems a bit stronger!";
			now M is notebook-studied.
		
	
[This may have been balanced, but it wasn't very fun.]
...
[Considering the player usually has to go three regions to get to the notebook, then run back, the item should be fairly powerful from base]
To decide which number is the damage improvement of (W - a notebook):
	let X be 2;
	increase X by the number of notebook-studied monsters;
	if the noun is notebook-studied:
		increase X by 2;[if you're having a bad run, this can be a real life saver]
	increase X by the number of worn blue scrunchies;
	if W is cursed, decrease X by 3;
	if W is blessed, increase X by 2;
