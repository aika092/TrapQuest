...
A graphlink processing rule (this is the default graphlink processing rule):
	unless the candidate replacement command is "":
		...
		[Next 5 lines are just copypasted from Hyperlinks by Dannii Willis, and this seems to now work!]
...
To set a graphlink in (g - a g-window) identified as (newlinkid - an object) for (newitem - an object) from (gleft - a number) by (gtop - a number) to (gright - a number) by (gbottom - a number) as (linkaction - some text), ignoring redundant links:
	if not ignoring redundant links and there is a linkid of newlinkid in the Table of Graphlink Glulx Replacement Commands:
		choose row with linkid of newlinkid in the Table of Graphlink Glulx Replacement Commands;
		now g-win entry is g;
		now p-top entry is gtop;
		...
		now p-right entry is gright;
		now replacement entry is linkaction;
		now linkowner entry is newitem;
	otherwise if the number of blank rows in the Table of Graphlink Glulx Replacement Commands is greater than 0:
		choose a blank row in the Table of Graphlink Glulx Replacement Commands;
		now linkid entry is newlinkid;
		now g-win entry is g;
...
Table of Graphlink Glulx Replacement Commands
linkid	g-win	p-top	p-bottom	p-left	p-right	replacement	alt	action	linkowner
an object	a g-window	a number	a number	a number	a number	some text	number	stored action	object
with 500 blank rows
...
Check debugging graphlinks:
	...
	repeat through the Table of Graphlink Glulx Replacement Commands in reverse order:
		say "[linkid entry] ([g-win entry]): ([p-left entry], [p-top entry]) - ([p-right entry], [p-bottom entry]):[if there is a replacement entry] [replacement entry][end if][if there is an alt entry] [alt entry][end if][if there is an action entry] [action entry][end if][if there is a linkowner entry] [linkowner entry][end if][line break]";
