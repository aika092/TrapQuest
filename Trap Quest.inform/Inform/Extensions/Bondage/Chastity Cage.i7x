...
To compute unique summoning of (C - a chastity cage):
	if the size of penis > 0, now the size of C is the size of penis.
...
To say ClothingDesc of (C - a chastity cage):
	say "[if the player is possessing a penis]A tight cage for your penis that stops you from getting an erection or stimulating your penis in any way.[otherwise]A chastity belt that completely prevents access to your [vagina]. The back straps go over each ass cheek, allowing full access to your [asshole].[end if]";
...
To say ShortDesc of (C - a chastity cage):
	say "chastity [if the player is possessing a penis]cage[otherwise]belt[end if]".
...
To compute periodic effect of (C - a chastity cage):
	if the player is male:
		...
		otherwise if the size of C > the size of penis + 1:
			say "Your [ShortDesc of C] is now too big for your [ShortDesc of penis] and falls to the ground.";
			now C is in the location of the player.
...
This is the can't cage ken rule:
	if the player is sexed male and the player is not possessing a penis:
...
This is the players in chastity can't wear vagina plugging clothes rule:
	if the player is possessing a vagina and there is a worn chastity cage and wearing-target is vagina plugging:
