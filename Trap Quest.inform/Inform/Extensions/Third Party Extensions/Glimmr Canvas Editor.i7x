Version 1/100805 of Glimmr Canvas Editor (for Glulx only) by Third Party Extensions begins here.
...
Include version 9 of Flexible Windows by Third Party Extensions.
Include Glimmr Canvas-Based Drawing by Third Party Extensions.
Include Glimmr Graphic Hyperlinks by Third Party Extensions.
Include version 5/100706 of Dynamic Objects by Third Party Extensions.
Include Basic Screen Effects by Third Party Extensions.
Include Undo Output Control by Third Party Extensions.
Include Glulx Status Window Control by Third Party Extensions.
Include version 4 of Questions by Third Party Extensions.
...
To decide what text is the extension-name of (typeface - a font):
	if the typeface is Glimmr Lucidex:
		decide on "Glimmr Image Font by Third Party Extensions";
	if the typeface is Glimmr C&C:
		decide on "Glimmr Bitmap Font by Third Party Extensions";
	decide on "".
...
Section - New real number question routine (in place of Section 7 - Processing real number questions in Questions by Third Party Extensions)
...
To write the/-- preamble:
	append "Include Glimmr Canvas-Based Drawing by Third Party Extensions.[if we have rendered strings][line break]Include Glimmr Text-Painting Elements by Third Party Extensions.[font extensions][end if][paragraph break]" to the file of Output;
...
GCE includes all of the other Glimmr extensions you will need, so usually you will need to include only GCE:
	...
	Include Glimmr Canvas Editor by Third Party Extensions.
...
Colors in Glimmr are used primarily for the backgrounds of windows, and for the colors of drawn elements such as rectangles, lines, and text elements. There are also a couple of UI elements within GCE that can be set using glulx color values. GCE and Glimmr Canvas-Based Drawing--which is the extension that GCE compiles its code to work with--use "glulx color values" to specify colors. The glulx color value type is defined in Third Party Extensions's Glulx Text Effects extension, and it provides a color name to stand in for far less grok-able color numbers that Glulx understands. By convention, glulx color values are written as a single (run-together) word prefaced with "g-".
...
Glimmr Canvas Editor requires exactly one bitmap font and one image font to work. By default, these are the fonts available in the Glimmr Bitmap Font and Glimmr Image Font extensions. However, you may change the fonts by replacing the Fonts chapter section of the GCE extension. Here is an example:
	...
	Chapter - My Fonts (replaces Chapter - Fonts in Glimmr Canvas Editor by Third Party Extensions)

	Include Bubble Blob Bitmap Font by Third Party Extensions.
	Include Gloopy Glop Image Font by Third Party Extensions.

	To decide what text is the extension-name of (typeface - a font):
		if the typeface is Gloop Glop:
			decide on "Gloopy Glop Image Font by Third Party Extensions";
		if the typeface is Bubble Blob:
			decide on "Bubble Blob Bitmap Font by Third Party Extensions";
		decide on "".
