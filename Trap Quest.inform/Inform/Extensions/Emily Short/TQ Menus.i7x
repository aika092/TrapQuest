TQ Menus by Emily Short begins here.
...
Current menu scroll is a number that varies. Current menu scroll is 1.
...
This is the move down rule:
	...
	if current menu selection is greater than current menu scroll + maximumMenuSize, increase current menu scroll by 1;
...
This is the move up rule:
	...
	if current menu selection is less than current menu scroll, now current menu scroll is current menu selection;
	reprint the current menu;
	make no decision.
...
To reprint (selected menu - a table name):
	...
	let __index be 0;
	let finalEntry be current menu scroll + maximumMenuSize;
	clear only the main screen;
	say "[if current menu scroll > 1]---MORE---[end if][line break]";
	repeat through selected menu:
		increase __index by 1;
		if __index >= current menu scroll and __index <= finalEntry:
			if __index is current menu selection, say " >";
			otherwise say "  ";
