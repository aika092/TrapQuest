Menus by Miscellaneous Frontend begins here.

"Aika's hacked version of Emily Short's table-based way to display full-screen menus to the player."
...
title	subtable (table name)	description	toggle	char-shortcut
"foo"	--	"bar"	a rule	a number
...
To show menu contents:
	...
	begin;
		...
		[ HACK to allow selecting a menu item by shortcut ]
		otherwise;
			if __x is at least 97 and __x is at most 122, decrease __x by 32; [ we make sure this is upper case ]
			if __x is a char-shortcut listed in the current menu [ is this check necessary, or can we just loop? ]
			begin;
				let __n be 0;
				repeat through current menu
				begin;
					increase __n by 1;
					if there is char-shortcut entry and char-shortcut entry is __x
					begin;
						now current menu selection is __n;
						follow the select rule;
						if temporary depth > menu depth
						begin;
							now __index is 1;
						end if;
...
hint	used	char-shortcut
"Sample Hint"	a number	a number
...
The displaying activity displays whatever is set as the current menu, so we must set the current menu before activating the activity. Afterwards it is a good idea to clear the screen before returning to regular play.
