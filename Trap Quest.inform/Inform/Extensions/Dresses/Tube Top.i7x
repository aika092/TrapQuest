...
To decide which object is the potential-upgrade-target of (C - a tube top):
	if C is sheer-when-wet, decide on a random off-stage sheer tube top;
	if C is actually sheer, decide on a random off-stage yellow exposing tube top;
	decide on nothing.
...
This is the remove inappropriate tube tops rule:
	repeat with B running through tube tops:
		unless B is cyan tube top or B is tartan tube top, now B is in Holding Pen.
...
Figure of cyan tube top is the file "tubetop1.png".
...
To say ClothingDesc of (C - a cyan tube top):
	say "A tiny cyan boob tube that clings tightly to your chest in order to stay in place.[if C is worn by player][line break]Your erect nipples can be clearly seen through it.[end if]".

To say ShortDesc of (C - a cyan tube top):
	say "tiny cyan boob tube".
...
Figure of silver tube top is the file "tubetop3.png".

To decide which figure-name is clothing-image of (C - a silver tube top):
	decide on figure of silver tube top.

To say ClothingDesc of (C - a silver tube top):
	say "A shiny silver boob tube that slings over one shoulder and clings tightly to your chest in order to stay in place.".

To say ShortDesc of (C - a silver tube top):
	say "tiny silver boob tube".

To decide which number is the initial outrage of (C - a silver tube top):
	decide on 2.

To decide which object is the potential-upgrade-target of (C - a silver tube top):
	decide on a random off-stage sheer-when-wet tube top.
...
Figure of pink rubber tube top is the file "tubetop7.png".
...
To decide which object is the potential-upgrade-target of (C - a pink rubber tube top):
	decide on a random off-stage sheer-when-wet tube top.
...
Definition: a tartan tube top (called C) is untransformable:
	if there is a worn scrunchie, decide yes;
	decide no.

To decide which object is the potential-upgrade-target of (C - a tartan tube top):
	if there is a worn scrunchie, decide on nothing;
	decide on a random off-stage silver tube top.
...
Figure of Red Tube Top is the file "tubetop4.png".
...
Figure of pink tube top is the file "tubetop5.png".
...
To say ClothingDesc of (C - a pink tube top):
	say "A super thin pink boob tube that clings extremely tightly to your chest in order to stay in place.[if C is worn by player][line break]Your erect nipples can be clearly seen through it.[end if]".

To say ShortDesc of (C - a pink tube top):
	say "tiny pink boob tube".
...
Figure of yellow exposing tube top is the file "tubetop6.png".
...
To say ClothingDesc of (C - a yellow exposing tube top):
	say "A yellow boob tube that is ripped in all the parts that matter, meaning that it fully exposes the wearer's chest.".
...
To decide which object is the potential-upgrade-target of (C - a jacket):
	decide on a random off-stage silver tube top.
...
Figure of rubber jacket is the file "latexjacket1.png".
