...
A pink rubber dress is a kind of overdress.  A pink rubber dress is usually unique.  A pink rubber dress is usually low cut.  A pink rubber dress is usually latex.  A pink rubber dress is usually short.
...
To compute periodic effect of (R - a pink rubber dress):
	if the player is not top heavy and a random number between 1 and 35 is 1 and the silicone volume of breasts < 20:
	...
	if the total volume of hips < max ass size and a random number between 1 and 35 is 1 and the silicone volume of hips > 0:
...
To say ShortDesc of (C - a pink rubber dress):
	say "stretchy rubber minidress that says 'PLASTIC IS FANTASTIC' across the front".

To decide which number is the initial outrage of (C - a pink rubber dress):
	decide on 11.

To decide which object is the potential-upgrade-target of (C - a pink rubber dress):
	if the player is not top heavy, decide on C;[wait, what?] [From Aika to whoever wrote this - look at the function below.]
	decide on nothing.
