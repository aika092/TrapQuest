...
This is the remove inappropriate maid outfits rule:
	repeat with B running through maid outfits:
		unless B is playdude maid outfit or B is cafe maid outfit or B is blue sissy maid outfit, now B is in Holding Pen.
...
To decide which object is the unique-upgrade-target of (M - a maid outfit):
	if there is a worn cafe maid headdress, decide on a random off-stage cafe maid outfit;
	decide on nothing.
...
To say ClothingDesc of (M - a playdude maid outfit):
	say "A black satin garment with only white straps for sleeves. [if diaper quest is 0]With a simple ruffled apron and a familiar logo printed on the skirt, this[otherwise]This[end if] sleek dress will catch eyes as you stand on your heels to dust the top shelf.".

To decide which number is the initial outrage of (C - a playdude maid outfit):
	decide on 7.

To decide which object is the potential-upgrade-target of (M - a playdude maid outfit):
	...
	decide on the potential-upgrade-target of C. [If the next target is not off-stage, we skip it.]
...
To decide which object is the potential-upgrade-target of (M - a drooping maid outfit):
	let C be a random cheap maid outfit;
	if C is off-stage, decide on C;
	otherwise decide on the potential-upgrade-target of C. [If the next target is not off-stage, we skip it.]
...
To decide which object is the potential-upgrade-target of (M - a cheap maid outfit):
...
To decide which object is the potential-upgrade-target of (M - a fetish maid outfit):
	...
	otherwise decide on the potential-upgrade-target of C. [If the next target is not off-stage or would clash with a skirt, we skip it.]
...
To say ClothingDesc of (M - a pink maid outfit):
	...
	if images visible is 1, display figure of maid outfit 5b;
...
To decide which object is the potential-upgrade-target of (M - a pink maid outfit):
...
Definition: a blue sissy maid outfit (called C) is sissifying:
	decide yes.

Definition: a blue sissy maid outfit (called C) is end of transformation chain:
	decide yes.
...
Definition: a cafe maid outfit (called C) is sissifying:
	decide yes.

Definition: a cafe maid outfit (called C) is end of transformation chain:
	decide yes.
