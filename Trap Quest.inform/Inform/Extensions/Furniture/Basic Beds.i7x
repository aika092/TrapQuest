...
Figure of bunk bed is the file "Env/Dungeon/bunkbed1.jpg".
...
To say ExamineDesc of (C - a bunk bed):
	say "A basic, inexpensive looking bed, for military personnel[one of]. You have no idea if you would get in trouble for resting on it or not[if the intelligence of the player > 7], but knowing this game, the odds aren't in your favour[end if][or][stopping].".
...
To compute rest ending of (F - a cot):
	if the number of modesty shutters in the location of the player > 0:
		...
		if number of plentiful accessories carried by M < 1, now a random off-stage plentiful ring is carried by M;
		interest M.
