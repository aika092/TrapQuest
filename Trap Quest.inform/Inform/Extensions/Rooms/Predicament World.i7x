...
Check going down when the player is in Park02:
	if current-predicament is gloryhole-predicament and the cocks-sucked of gloryhole-predicament < 5 and the mens-room-entered of gloryhole-predicament is 0:
		say "Remember, [bold type]you'll be punished if you go into the men's room before finishing sucking enough cocks.[roman type][line break]";
...
This is the bystanders move rule:
	...
	if the player is in an unbossed predicament room or the player is in Predicament02 or the player is in Toilet01 or the player is in Toilet02 or (current-predicament is vibe-photo-predicament and the player is in Predicament01):
...
To maths-execute (M - jizz-on-me):
	say "You hear the [man of shopkeeper] chuckle and whip out [his of shopkeeper] [manly-penis]. It's not long before you hear [him of shopkeeper] grunting and feel the warmth of [his of shopkeeper] seed on your [AssDesc]!";
	AnnouncedSquirt semen on hips by 2;
	say "After that, you hear [him of shopkeeper] leave the way [he of shopkeeper] came.".
...
To maths-execute (M - new-ankle-tattoo):
	...
	if diaper quest is 0, say "The [man of shopkeeper] leaves the way [he of shopkeeper] came, murmuring to [himself of shopkeeper] about a job well done.".
...
To execute (L - photo-display-predicament):
	...
	let M be a random off-stage normal-monokini;
	only destroy M;
				...
				otherwise:
					...
					repeat with XXX running from 1 to 3:
						...
						if diaper messing >= 3 and nowMessing is false:
							...
							let I be hold-strength - rectum-incontinence of the player;
							if debuginfo > 0 and rectum > 1, say "[input-style]Mess self-control check: d5+9 ([hold-strength]) - bowel incontinence ([rectum-incontinence of the player]) = [I + 0][if I < 4]; minimum 4[end if] | ([rectum].5) rectum volume[roman type][line break]";
							if I < 4, now I is 4;
							if rectum >= I:
								now nowMessing is true;
	...
	if team-blowjob-race-predicament is reeled-out:
		...
		otherwise:
			...
			otherwise if (1 is listed in the previous-sex-methods of team-blowjob-race-predicament or 3 is listed in the previous-sex-methods of team-blowjob-race-predicament):
				...
				say "You're forced to slurp up the [if N > 1][N] loads of [end if]pussy juices that have been left behind on the bound [man of shopkeeper][']s [manly-penis].";
				TasteGrossOut N;
...
To say PredicamentDescription of (P - toilets-dildos-predicament):
	say "I'm currently in an Extra Credit lesson where I need to suck dildos in the women's toilets to open boxes to aleviate my predicament, with only 60 seconds until the glass goes transparent and reveals me, and then get back to my home while avoiding as many bystanders as possible from noticing me.".
...
An all later time based rule:
	if current-predicament is toilets-dildos-predicament and the player is in Toilet02:
		if the smart-glass-timer of toilets-dildos-predicament > 0:
			...
			if the smart-glass-timer of toilets-dildos-predicament <= 0:
				say "[bold type]The timer hits 0 seconds! The Smart Glass malfunctions, and turns transparent![roman type][line break]";
			otherwise if a random number between 1 and 2 is 1:
				say "You hear a voice from outside.[line break][second custom style]'[one of]Excuse me, I'm waiting!'[or]Is anyone even in there?!'[or]Finish up before I piss myself!'[or]You're taking forever!'[or]What's the hold up?!'[or]Are you okay in there?!'[in random order][roman type][line break]";
		if the smart-glass-timer of toilets-dildos-predicament > 0:
...
[balloon-roomba-predicament is a team-predicament. balloon-roomba-predicament has a number called player-balloons. balloon-roomba-predicament has a number called rival-balloons.]
		...
		if player-released is false:
			...
			while total-men > 0:
				...
				if rival-released is false and man-balance > 0:
					say "[if man-balance is 2]Next, the other [man of N] has [his of N] turn, until [he of N] also[otherwise]At around the same time, the other [man of N][end if] climaxes inside [NameDesc of team-predicament-partner], and pulls out with a satified grunt.";
					if T matches the text "PLUGGED":
		...
		if diaper quest is 0:
			...
			if M is a thing:
				if M is not penetrating a body part:
					...
					zero focus stuff;
			otherwise if the player is in Toilet02:
				if business-briefcase-predicament is not predicament-failed and house-key is in Toilet01 and a random number between 1 and 3 is 1:
					let M be a random ultimate-lesson-actor;
					set up M;
					now M is in Toilet02;
					now M is not interested;
							...
							if M is interested and the player is getting unlucky:
								...
								if player-numerical-response is 1:
									...
									if player-numerical-response > 0: [currently all outcomes. eventually I might add some sexual favour demands]
										say "[BigNameDesc of M] looks at you skeptically.";
										[if M is male:
