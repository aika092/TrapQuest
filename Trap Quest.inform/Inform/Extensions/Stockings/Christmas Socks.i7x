...
To decide which object is the potential-upgrade-target of (C - a deer socks):
	decide on a random frilly green socks.
...
To decide which object is the potential-upgrade-target of (C - a frilly green socks):
	decide on a random off-stage woven stockings.

Definition: a frilly green socks (called C) is untransformable:
	if there is a worn santa hat, decide yes;
	decide no.


Christmas Socks ends here.
