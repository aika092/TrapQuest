...
Definition: a demoness is willing to do vaginal:
...
To say SoulDesc of (M - a demoness):
	if the soul-timer of M < -99, say "greater ";
...
To say MonsterDesc of (M - an abyssal demoness):
	say "An aura of terrifying evil surrounds this woman. The demoness has a statuesque humanoid figure but her skin is a rich sea blue and although her face is slightly obscured by a black silk veil, you can tell her eyes are completely yellow. The air around her seems to shift and move as if covered in a fine layer of constantly moving water, and two curved horns give her head a decidedly aquatic profile. [if pregnancy fetish is 1]She is completely naked except for a pair of tall black heels with glass dildos for heels, and at least 3 months pregnant[otherwise]She is wearing an extremely provocative black bikini top, tall black high heels with glass dildos for heels, and a matching nylon miniskirt, which only barely covers her crotch[otherwise if diaper quest is 1] and shiny red clothing[end if][if doomed is 5]. Pink lightning ominously crackles around her...[otherwise].[end if]";
...
To decide which number is the maxhealth of (M - a demoness):
	...
	if the soul-timer of M is -99, increase X by 50;
	decide on X.
...
To decide which number is the semen load of (M - a demoness):
	let X be 1;
	increase X by the excitement of M;[if you don't resist, you have to deal with a much smaller load]
	if the soul-timer of M is -99, increase X by X.
...
To DifficultyUp (M - a demoness) by (X - a number):
	while X > 0:
		if the soul-timer of M is -99 and a random number between 1 and 2 is 1, increase the difficulty of M by 1;
...
Definition: a demoness (called M) is objectifying the player:
	if the class of the player is cheerleader or M is not in the location of the player, decide no;
...
Carry out kneeling in the presence of a demoness:
	repeat with M running through dangerous demonesses in the location of the player:
		...
		otherwise if the channelling of M is 1:
			...
			HairRedUp 1;[###MG Is this meant to happen?]
...
To compute anal sex of (M - a demoness):
	...
	if the reaction of the player > 0:
		...
		if the reaction of the player is 2:
			decrease the difficulty of M by 2;
		otherwise:
...
This is the demoness doesn't care for skinny bitches rule:
	...
	if the player is souled and (the class of the player is priestess or the class of the player is virgin warrior or the virgin bonus of the player > 0):
		...
		say "[BigNameDesc of M] places [his of M] hand on your chest and begins to chant, and a pronounced empty feeling settles in your core as a bright blue gem materializes in [his of M] hand! [big he of M] then walks off, laughing.";
		bore M;
...
To compute (M - a demoness) attacking (C - a clothing):
	...
	if the soul-timer of M is -99, now R is 99;
...
To compute (M - a demoness) entering anally:
	...
	cutshow figure of demoness cutscene 2 for M;
...
To compute (M - a demoness) entering mouth:
	...
	if face is not actually occupied:
		...
		if the soul-timer of M is -99, increase the sex-length of M by 1;
...
Definition: A demoness (called M) is willing to bukkake:
	if the player is not a blowjob slut or the soul-timer of M is -99, decide no;
...
This is the demoness friendly convinced rule:
	...
	if presented-orifice is a breasts:
		if the player is soulless:
			say "[speech style of M]'[one of]I already have your soul. I can't take it again.'[or]Yes, your heart is an empty, soulless void. You're welcome.'[or]I have your soul already, fool.'[at random][roman type][line break]";
		otherwise if the virgin of the player is 1 and the player is female:
		...
		otherwise:
			...
			if the player is consenting:
				say "[BigNameDesc of M] smirks, placing [his of M] hand on your chest and beginning to chant. A pronounced [i]empty[/i] feeling settles in your core as a bright blue gem materializes in [his of M] hand! [big he of M] then walks off, laughing.";
				FavourUp M;
				compute M taking soul;
			otherwise:
...
To compute soul creampie of (M - a demoness) in (F - vagina):
	say "[BigNameDesc of M] hilts herself deep inside your [vagina], cackling madly as she floods you with her load. An instant chill, like a candle being blown out, falls over your body as her [semen] flows into your [vagina], and a bright blue gemstone materializes in the palm of [his of M] outstretched hand. [speech style of M]'Could you feel, that, baby? I just fucked that juicy soul straight out of your body, and you just let it happen! Hahaha! I hope it was worth it!'[roman type][line break][big he of M] cackles madly as [he of M] pinches your clit between two ominously glowing fingertips, causing powerful waves of pleasure to crash through your body. You feel incredible, but you can't help but feel [if the virgin bonus of the player < 0]a second, stronger feeling of shame[otherwise]second, stronger feeling of emptiness[end if] settling heavily in your chest.";
		...
		if M is awake:
			...
			otherwise:
				...
				otherwise:
					...
					if the soul-timer of M is -99, increase the excitement of M by 1;
					increase the excitement of M by 1;
		otherwise:
			anger M;
			now M is interested;
...
To compute unique death of (M - a demoness):
	let R be a random number between 1 and 5;
	say "The [noun] screams as [he of M] falls to the floor, and then you watch as [his of M] essence is sucked back into [his of M] home dimension.";
...
To compute punishment of (P - demoness-facesit):
	...
	if the number of worn knickers is 0 or (the player is diapered and D is DQBulkier):
		...
		UrineSoakUp D by 10;
		if diaper messing >= 7, MessSet D to 20;
	satisfy M.
...
To compute forcefeed round (N - a number) of (M - a demoness):
	...
	if diaper messing >= 3 and a random number between 1 and diaper lover > 1 and (the player is not thirsty or the stomach-food of the player < the stomach-liquid of the player):
		...
		StomachFoodUp 1;
	otherwise:
		say "a floating, swirling ball of [one of]some kind of fruit juice[or]warm milk[or]warm black tea[in random order] out of thin air and makes it shoot into your mouth. You are forced to swallow it all down.";
		StomachUp 1.
...
To say FirstResponse of (M - a demoness):
	...
	appropriate-display figure of dildo heels.
...
To compute taunt acceptance effect of (M - a demoness):
	increase the excitement of M by 1;
	increase the difficulty of M by 6;
	regionally place M.
...
To decide which number is the mild-annoyance threshold of (M - a demoness):
	decide on 150.[The demoness is not very tolerant of mortals]
...
To compute teaching of (M - a demoness):
	...
	teach strutting;
	SlightConvoFatigue M.[She is always happy to talk about her heels]
...
To suggestFucker (F - a body part) for (M - a demoness):
	if F is penis:
		if watersports fetish is 1 and the player is desperate to pee:
			say "Do you want to give [him of M] a golden shower? It could be really difficult...";
			if the player is consenting:
		...
		if player-fucker is face:
			say "Do you want to try putting your [SexDesc of penis] in [his of M] [HoleDesc of M]? [if watersports fetish is 1 and the player is desperate to pee]It still wouldn't be easy[otherwise]It could be hard[end if]...";
			if the player is consenting:
				now player-fucker is F;
	...
	otherwise if F is a fuckhole:
		if F is vagina and watersports fetish is 1 and the player is desperate to pee:
			...
			if the player is consenting:
				...
				now player-fuckchoice is FUCK-WATERSPORTS;
		if player-fucker is face:
			say "Do you want to have [him of M] eat out your [variable F]?";
			if the player is consenting:
				now player-fucker is F;
...
To compute failed dominance punishment of (M - a demoness):
	if the class of the player is priestess or the player is not able to speak or the times-dominated of M > the times-submitted of M:[summons imps to gangbang you]
		say "[speech style of M]'Hmmm, what should I do with you now? I was already planning to make you cry, but [if the times-dominated of M < 1]now I'll have to think of something more creative.[otherwise]since you have such high regard for my services, I have to do better than that. Don't you agree?[end if]'[roman type][line break][BigNameDesc of M] whistles sharply and at least a dozen tiny red [if lady fetish is 1]women[otherwise]men[end if] stampede [if playerRegion is Woods]out of the under brush[otherwise]through the door[end if] and form a loose circle around you as [NameDesc of M] steps away. [line break][speech style of M]'While I think about it, would you mind entertaining my associates for a little while?'[roman type][line break]The imps don't waste any time crowding in around you, [unless there is a worn wrist bond]tying your hands behind your back[otherwise]holding you down[end if] and [if the number of worn stealable clothing > 1]stripping off every piece of clothing they can[otherwise if there is a worn stealable clothing]stripping off your [printed name of a random worn stealable clothing][otherwise]snickering mischievously[end if] as they grope and fondle your body.";
		if lady fetish is 1:
		...
		otherwise:
			say "Being surrounded by so many naked men [if the player is male]makes you feel like you accidentally stumbled into a locker room, and[otherwise]makes you feel almost special, but[end if] as the first imp slides his [manly-penis] into your mouth, it becomes totally clear that they see you as nothing more than a cheap cocksleeve. There's little time to breathe or even swallow after the first imp is finished, another imp immediately takes [his of demon lord] place. [if the player is male]Your [asshole] isn't[otherwise]Your holes aren't[end if] exempt from the treatment, and the imps take turns racing each other to see which end they can fill faster.";
			let X be 5;
			...
			StomachSemenUp X;
			let N be M;
			if lady fetish is 1, let N be a random vine;
			otherwise let N be demon lord;
	...
	otherwise:
		...
		otherwise:
			...
			otherwise if watersports fetish is 1:
				say "[speech style of M]'Here's something for YOU to remember.'[roman type][line break][BigNameDesc of M] directs your face towards [his of M] crotch and lifts up [his of M] dress, releasing a torrent of piss over your hair and face.";
				FacePiss from M;
			otherwise:
...
To vaginal dominate (M - a demoness):
	...
	if the sexual-penis-length > 8:
		say "[line break][speech style of M]'All of this rough-housing has me feeling pretty hot. Scorching hot, even. If you[']re smart-'[roman type][line break][big he of M] pauses mid-statement to grin maliciously, and you realize it's going to be forever before you get to fuck [him of M] if you keep listening. Oh well. You [if the player is a bit horny]slam[otherwise]slide[end if] your [SexDesc of penis] into [his of M] tight [HoleDesc of M] without waiting for [him of M] to continue, giving [his of M] taut [AssDesc] a good slap as you begin to thrust. [big he of M] shrieks in indignation, flinging curses at you as the heat radiating off her body grows from pleasant to mildly uncomfortable. You faintly remember something she said about 'scorching' you, but the harder you pound [him of M], the easier the heat is to deal with. [BigNameDesc of M] scowls at you as you violently jackhammer away, and the warmth from her furnace-like [HoleDesc of M] sinks through to your core, rapidly intensifying the pleasure until your balls thoroughly empty themselves inside [him of M].";
		obsceneDignify;
		orgasm;
		FavourDown M by 2;
...
To oral dominate (M - a demoness):
	...
	otherwise:
		...
		otherwise:
			say "[if the player is not able to speak]You shake your head. [otherwise if the sex addiction of the player < 7][line break][variable custom style]'I[']m not letting you trick me.'[roman type][line break][otherwise if the sex addiction of the player < 12][line break][variable custom style]'Sorry, I just really need this.'[roman type][line break][otherwise][line break][variable custom style]'Come on, it[']ll be a lot of fun...'[roman type][line break][end if][BigNameDesc of M] nods, as [he of M] slips between your legs, chuckling to [him of M]self as [he of M] gently pushes your [AssDesc] apart. You feel [his of M] nails dig into you as [he of M] firmly plants [his of M] lips on your anal ring, and ripples of heat spread out around it. [big he of M] teases the inner edges of your sphincter with [his of M] tongue as arousal rapidly builds up inside you, making sure to thoroughly cultivate your frustration... only to suddenly duck under you and shove you away!";
...
To watersports dominate (M - a demoness):
	say "You grab [NameDesc of M] and quickly realize [he of M]'s a lot stronger than you, but with enough effort you manage to wrestle [him of M] to [his of M] knees. [big he of M] curls [his of M] lip.[line break][speech style of M]'You plan on making me your toilet, but that isn[']t smart. Release me now or feel my wrath.'[roman type][line break]Release her?";
	if the player is consenting:
...
Definition: abyssal demoness is mansion dwelling: decide yes.
