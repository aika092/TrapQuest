...
To say MonsterDesc of (M - a demoness):
	if lady fetish is 2:
		say "An aura of terrifying evil surrounds this man. His lithe, statuesque features are [if diaper quest is 1]covered up by shiny red clothing[otherwise]a bit too perfect, and so much heat is emanating from him that the air around him slightly shimmers. He is wearing an extremely provocative shiny red dress and tall black heels with glass dildos for stilettos. Two red horns rise up from his long blonde hair[end if][if doomed is 5]. Pink lightning ominously crackles around him...[otherwise].[end if]";
	otherwise:
...
To compute perception of (M - a demoness):
	...
	otherwise if the player is prone:
		...
		otherwise:
			say "[speech style of M]'Keep crawling, [boy of the player], nothing to see here.'[roman type][line break]";
			distract M;
...
This is the demoness punishes diaper rule:
	...
	if P is a thing:
		...
		if the size of P < a random number between 6 and 10:
			say "Smirking, [NameDesc of current-monster] uses [his of current-monster] infernal magic to make your [P] grow inside of your [asshole]!";
		...
		otherwise:
			say "With a snap of [his of current-monster] fingers, the [P] begins twisting, shaking and vibrating violently! Your [if the anal sex addiction of the player < 5]poor [end if][asshole] is powerfully stimulated.";
	...
	otherwise if M is uniquely unfriendly:
		...
		otherwise:
			...
			if a random number between 1 and 15 < the bimbo of the player and the player is upright and the player is not dildo stuck:
				...
				try kneeling;
	otherwise if the player is prone:
		say "[speech style of M]'[if the class of the player is cultist]Servant.'[otherwise if the class of the player is succubus]Good practice, sister. Glad to see some of his servants aren't idiots.'[otherwise]Get used to crawling, [boy of the player], all will be servants eventually.'[end if][roman type][line break]";
		distract M;
