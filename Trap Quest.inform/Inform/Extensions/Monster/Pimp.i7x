...
A pimp is a kind of monster. A pimp is usually intelligent. A pimp is male.

Definition: a pimp (called M) is father material:
	decide yes.

Definition: a pimp (called M) is dark skinned:
	decide yes.

Definition: a pimp (called M) is raunchy:
	decide yes.

pimp-dead is a number that varies.

Definition: a pimp (called M) is willing to do oral:
	decide no.

Definition: a pimp (called M) is willing to do titfucks:
	decide no.
...
To compute labour to (M - a pimp):
	if M is regional and M is alive:
		...
		say "[PregFlav][if the father is in the location of the player]The [father][otherwise]The [father] appears almost as if on cue!  He[end if] kneels down on one knee and delivers the human mixed race baby, which immediately starts crying loudly. He takes it into his arms with an unconcealed look of annoyance. [line break][first custom style]'Geez, thanks a lot!  Now I have to manage a brothel AND raise [one of]a[or]yet another[stopping] fucking kid.'[roman type][line break]Without giving you a chance to react, never mind reply, he leaves you to recover from your ordeal.";
...
To compute kneeling reaction of (M - a pimp):
	say "The [M] laughs. [line break][first custom style]'[if the health of M >= the maxhealth of M]I guess this one time, you can pay me with your services[otherwise]Surrender means nothing to me. You've crossed me, and now I'm going to make your life a misery[end if].'[roman type][line break]";
	humiliate 75.

To say DiaperReaction of (M - a pimp):
	say "The [M] laughs. [line break][speech style of M]'That's right, little one. Prove to me what a good little baby-slave you are.'[roman type][line break]";
...
To say PresentRejectionFlav of (M - a pimp):
	say "The [M] cuts you off before you've finished talking. [line break][speech style of M]'I own you, I'll fuck you wherever I goddamn please.'[roman type][line break]".

To say SelectionFrustrated of (M - a pimp):
	say "The [M] laughs, seemingly at himself. [line break][speech style of M]'For some reason I can't seem to get it up. Fine, I guess you get off lucky. This time. You'd better remember this very rare moment of kindness I'm showing you, whore.'[roman type][line break]";
	satisfy M.
...
To say SatisfiedFlav of (M - a pimp):
	say "The [M] grunts with satisfaction. [line break][speech style of M]'That'll do as payment for now.'[roman type][line break]He walks away, leaving you to [if the bimbo of the player < 12]contemplate the consequences of your choices[otherwise]bask in the afterglow[end if].".
...
To compute friendly drink of (M - a pimp): [Should never happen]
	if M is interested:
		say "The [M]'s smile fades slightly. [line break][first custom style]'Sorry, we're not allowed to give out free drinks, it is pretty much the only way the hotel make money after all!'[roman type][line break]";
	otherwise:
...
To say MonsterOfferAcceptFlav of (M - a pimp) to (T - a thing):
	say "The [M] smiles. [line break][speech style of M]'[one of]Good girl[or]That's my good little [if diaper quest is 1]baby[otherwise]whore[end if][or]Keep it up[in random order].'[roman type][line break]".
...
To say MonsterOfferRejectFlav of (M - a pimp) to (T - a thing):
	say "[if M is unfriendly]The [M] smiles.[otherwise]The [M] looks a bit confused.[end if]  [line break][speech style of M]'[if T is plentiful accessory and the health of M < the maxhealth of M]It's too late for that, bitch.'[otherwise]Why the hell would I want that?'[end if][roman type][line break]";
