...
To say DiaperReaction of (M - a demon lord):
	say "The [M] laughs, with a deep booming tone.[line break][speech style of M]'[one of]YES, I SEE YOU ARE LEARNING YOUR TRUE ROLE AS A PATHETIC DIAPERED MORTAL[or]YOU ARE SO PATHETIC THAT YOU CANNOT EVEN WAIT TO BE ALONE BEFORE SOILING YOUR OWN GARMENTS[at random]!'[roman type][line break]".
...
To compute punishment of (P - xavier-nightmare-gag):
	...
	if K is clothing and K is not an unmessed dry plain diaper and K is not a demon diaper:
		say "[if K is diaper and the DQbulk of K < 7][second custom style]'That's not near big enough!'[roman type][line break][end if]She points to your [K], and before you can even react, a huge slice appears all the way across it, and it falls from your body in tatters.";
		if K is messed: [need to check before we destroy and reset it]
			destroy K;
	...
	if G is actually summonable, summon G cursed;
