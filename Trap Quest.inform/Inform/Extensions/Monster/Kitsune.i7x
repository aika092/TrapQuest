...
To say KitsuneName:
	if the target-disguise of item described is item described, say "[if item described is in the location of the player][TQlink of item described][end if][input-style]kitsune[if the sleep of the item described > 0] (fast asleep)[end if][shortcut-desc][roman type][if item described is in the location of the player][TQxlink of item described][verb-desc of item described][end if]";
...
The description of kitsune is usually "[KitsuneDesc]".
...
To set up (M - a kitsune):
	now the monstersetup of M is 1;
	now the difficulty of M is 12;
	now the health of M is the maxhealth of M;
	set up disguise of M.

To set up disguise of (M - a kitsune):
	let R be a random number between 1 and 2;
	if R is 1:
		now the target-disguise of M is a random vampiress;
	otherwise:
...
To reveal disguise of (M - a kitsune):
	if M is in the location of the player, say "Standing in place of the [M] is now a ";
...
This is the spawn initial kitsune rule:
	if the number of alive kitsune is 0:
		let M be a random kitsune;
		set up M;
...
Definition: a kitsune (called M) is objectifying the player:
	decide no.
	
To compute perception of (M - a kitsune):
	now M is interested;
	if the target-disguise of M is M:
...
To say WhoAnswer of (M - a kitsune):
...
To say StoryAnswer of (M - a kitsune):
