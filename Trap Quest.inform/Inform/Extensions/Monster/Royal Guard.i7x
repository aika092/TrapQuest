...
To say MonsterDesc of (M - a royal guard):
	...
	otherwise:
		say "A calm, dignified, but obviously powerful woman, with hints of an impressive bust as well as a healthy musculature visible within [his of M] thick robes, which bear a royal crest. [big he of M] has [GuardHair of M] hair drawn back into a neat ponytail that rests over her shoulder.";
...
Definition: a royal guard is manly-wenchy: decide yes.
...
To say OfferThanksFlav of (M - prison guard) for (T - a thing):
	say "[speech style of M]'[if skeleton key is off-stage]I guess you can have this, as long as nobody finds out[otherwise]I would give you something, but I have actually recently misplaced it[end if].'[roman type][line break]".
...
Definition: a royal guard (called M) is objectifying the player:
	...
	if A >= 20 or A > the bimbo tolerance of M, decide yes;
	decide no.
...
To compute monstermotion of (M - a royal guard):
	...
	otherwise if the class of the player is priestess and the player is in the location of dungeon altar and the refactory-period of M <= 0:
		check seeking 1 of M;
...
To compute unique periodic effect of (M - a royal guard):
	if diaper quest is 0 and M is undefeated and M is awake and the refactory-period of M <= the refactory-time of M * -1 and M is not penetrating a body part:[is he horny?]
		if a random number between 1 and 45 is 1:
			...
			now the refactory-period of M is -4.[He'll only be completely satisfied by sex]
...
To compute post climax effect of (M - a royal guard) in (F - a fuckhole):
	...
	FavourUp M;
	if the class of the player is princess or M is prison guard, compute happy reward of M;
	reset submissive wenches;
	bore M.
...
This is the royal guard punishing a criminal rule:
	...
	if there is stolen clothing and shopkeeper is unfriendly and milking is 0 and S is in the location of M:
		...
		if S is not interested, compute perception of S;
...
This is the royal guard arresting a criminal rule:
	let M be current-monster;
	if there is held stolen clothing and shopkeeper is unfriendly and milking is 0 and shopkeeper is alive and shopkeeper is not in the location of the player:
...
To compute unique death of (M - a royal guard):
	...
	if full-lady fetish is 1 and a random number between 1 and 3 is 1 and the player is the donator:
		...
		now the strap-length of X is 7;
	otherwise if R > 3:
	...
	if M is prison guard:[the skeleton key overrides other drops]
		if skeleton key is off-stage, now X is skeleton key;
...
To say RepeatResponse of (M - a royal guard):
	...
	otherwise if player is exposed:
...
To say SubmissiveResponse of (M - a royal guard):
	...
	otherwise if M is unfriendly:
		...
		otherwise:
			say "[speech style of M]'[one of]Appalling. I will enjoy using you.[or]I'm glad you've finally realized your place.[or]Like a true whore you accept your place.[or]That's a good whore. Now, it is time you sated my urges.[at random] [run paragraph on]";
	...
	otherwise:
		if the class of the player is princess:
			say "[speech style of M]'[one of]It was an honour to serve you, my leige.'[or]Please do not hesitate to call on me again, your highness.'[or]I am forever at your service, your majesty.'[at random][roman type][line break]";
		otherwise if the class of the player is royal slave:
			say "[speech style of M]'[one of]It seems that you are finally realizing your place.'[or]Ha. Do not worry, slave. I plan on using you again soon.'[or]I am glad to see you dedicate yourself so fully to the crown.'[at random][roman type][line break]";
		otherwise:
...
To say TauntRejected of (M - a royal guard):
	if the class of the player is princess:
		say "[speech style of M]'[one of]I don't think so princess. Its time you saw to MY needs.'[or]Why don't we get on our knees together, princess.'[or]I apologize, your highness, but I cannot let you have your way. I must handle this properly.'[at random][roman type][line break]";
	otherwise:
...
To decide which number is the mild-annoyance threshold of (M - a royal guard):
	if the class of the player is princess, decide on 390;[he is more patient with the royal family]
	decide on 240.
...
To compute annoyance of (M - a royal guard):
	...
	otherwise if the questioned of M > the high-annoyance threshold of M and the class of the player is princess:
	...
	otherwise if the questioned of M > the high-annoyance threshold of M and B is actually summonable and tutorial is 0:
		say "[speech style of M]'Enough questions! Won't you be quiet now? Here, maybe this will help.'[roman type][line break]The royal guard pulls out a ballgag and fastens it to your mouth before you can react! [line break][variable custom style]'Mmmmph!'[roman type][line break]";
		summon B locked;
	otherwise:
		say "[speech style of M]'Enough questions. I must continue to patrol!'[roman type][line break]";
		ApplyMaxConvoFatigue to M.
...
To compute unfriendly drink of (M - a royal guard):
	...
	otherwise if face is not actually occupied and (the semen taste addiction of the player > 6 or the player is not modest or the thirst of the player is 5):
...
To compute desperate drinking to (M - a royal guard):
	say "You [if the player is upright]drop to your knees and [end if]beg [NameDesc of M] to give you something to drink.[line break]";
	if M is unfriendly or the favour of M < the saved appearance of the player:
		now M is interested;
		anger M;
		now the stance of the player is 1;
		compute M entering mouth;
...
To suggestFucker (F - a body part) for (M - a royal guard):
	if F is penis:
...
To say DominanceFailure of (M - a royal guard):
	if player-fucker is penis:
		say "You try to force [NameDesc of M] the ground, but your apparent difficulty overpowering [him of M] makes it perfectly clear who will be dominating who. It's hard not to notice the tent in [his of M] robes as [he of M] forces you onto your hands and knees.";
	otherwise:
...
To compute failed dominance punishment of (M - a royal guard):
	if the class of the player is royal slave and watersports fetish is 1:
		say "[speech style of M]'You will pay dearly for this transgression, wench.'[roman type][line break][BigNameDesc of M] grabs a handful of your hair, forcing you to turn and look up at [him of M]. [big he of M] glares down at you sternly as [he of M][if M is wrapped]peels off the condom and pisses[otherwise] begins peeing[end if] in your face.";
		now M is unwrapped;
		FacePiss from M;
...
To vaginal dominate (M - a royal guard):[with times-submitted we'll assume the sex you had was the "punishment" kind]
	...
	otherwise:
		let S be (the soreness of vagina * 2);
		increase S by (O - G) * 2;
		let A be the semen addiction of the player;
		let R be a random number between 10 and 17;
		decrease R by the arousal of the player / 450;
		if debugmode > 0, say "[input style]Guard pull off check: Is [S] (sensation roll) or [A] (semen addiction) < [R] (Arousal roll)?[roman type]";
		if S < R or A < R:
			...
			if the player is bimbo consenting:
				...
				strongDignify;
			otherwise:
				say "It's not [his of M] lucky day. You stop abruptly and pull [him of M] out, relishing [his of M] look of frustration as you return to your feet. [if watersports fetish is 1 and the player is desperate to pee]Do you want to humiliate [him of M] further with a golden shower?[end if]";
...
To anal dominate (M - a royal guard):
	...
	if M is wrapped:
		...
		say AfterDominationComment 1 of M;
	otherwise:
		let S be (the soreness of asshole * 2);
		increase S by (O - G) * 2;
		let N be the semen addiction of the player;
		let R be a random number between 10 and 17;
		decrease R by the arousal of the player / 450;
		if debugmode > 0, say "[input style]Guard pull off check: Is [S] (sensation roll) or [N] (semen addiction) < [R] (Arousal roll)?[roman type]";
		if S < R or N < R:
			if debugmode > 0, say "[bold type]PASSED[roman type][line break]";
			say "[line break]Do you want to keep going and let [him of M] creampie you?";
			if the player is bimbo consenting:
