...
To compute (M - a monster) stomping (N - a royal guard):[generic M here, but shouldn't matter.]
	if diaper quest is 1:
		if N is in the location of the player, say "[BigNameDesc of M] jumps on the [N], completely covering [his of N] body with its own. The [N] screams, and then... cries?  [BigNameDesc of M] stands back up to full height, revealing a wailing diapered man underneath, with a full diaper and a rattle instead of a sword.";
	...
	otherwise:
		if N is in the location of the player, say "[BigNameDesc of M] forcefully pushes the [N] to the ground and stamps on [his of M] body. The [N] is now motionless on the ground.";
...
To compute unfriendly drink of (M - a royal guard):
	...
	otherwise if face is not actually occupied and (the semen taste addiction of the player > 6 or the humiliation of the player > 16000 or the thirst of the player is 5):
		...
		humiliate 225;
	otherwise:
		say "You think about asking [if M is male]him[otherwise if M is female]her[otherwise][NameDesc of M][end if] for a drink, but then you realize what that might entail and decide that you're not desperate enough.";
	say "[line break]".
...
To vaginal dominate (M - a royal guard):[with times-fucked we'll assume the sex you had was the "punishment" kind]
	...
	otherwise:
		if ((watersports fetish is 1 and the player is desperate to pee) or the player is not very horny or (the soreness of vagina < a random number between G and 7) or (the semen addiction of the player < a random number between 5 and 12)) and the class of the player is not faerie:[faeries don't pull off, being pregnant is too much fun!]
			say "Unfortunately, you're in charge here, and you're not about to let [him of M] cum when you're not even close. You stop abruptly and pull [him of M] out, relishing [his of M] look of anger and confusion as you return to your feet.";
			if the player is desperate to pee:
				say "[speech style of M]'WENCH! YOU WILL FIN-'[roman type][line break]You're not really interested in what [he of M] has to say, so you cut [him of M] off by relieving your bladder all over [his of M] face, sighing loudly as [he of M] angrily raises [his of M] hands to try to block the flow.";
