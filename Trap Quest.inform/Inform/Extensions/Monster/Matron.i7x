...
matron is a monster. matron is intelligent. The difficulty of matron is 9. The leftover-type of matron is 116. matron can be motherly or unmotherly. matron has a number called feedingtone. matron can be diaper-prepared. [This means the matron has decided the player needs a diaper right now.]

Definition: matron (called M) is hotel dwelling:
...
Definition: matron (called M) is able to remove cursed plugs:
	decide yes.
...
Definition: matron (called M) is human:
	decide yes.
...
To say LeftoverDesc (N - 116):
	say "An unconscious [input-style][ShortDesc of matron][roman type] on the ground here has a bulging belly and still has [semen] slowly flowing out of her [vagina].[line break]".
...
To compute sudden objectification of (M - matron):
	say "[BigNameDesc of M] puts a hand over her mouth in shock. [line break][speech style of M]'Oh dear darling, this just won't do. You clearly need someone to take care of you...'[roman type][line break]".
...
To compute perception of (M - matron):
	...
	otherwise if the class of the player is schoolgirl and there is no worn diaper:
		...
		now M is diaper-prepared;
	otherwise if diaper focus is 1 or M is diaper-prepared:
...
Definition: matron (called M) is uniquely unfriendly:
	if M is diaper-prepared, decide yes;
	decide no.
...
Definition: matron (called M) is willing to change diapers:
	decide yes.

 Definition: matron (called M) is willing to spank:
	decide yes.

Definition: matron (called M) is willing to masturbate:
	decide yes.

Definition: matron (called M) is willing to deliver enemas:
	decide yes.
...
Definition: matron (called M) is eager to change diapers:
	...
	if M is diaper-prepared, decide yes; [this means the matron has decided the player needs a diaper right now.]
...
To compute diaper change chastity cummies reward of (M - matron):
	...
	diaperaddictup 1;
	say "[BigNameDesc of M] pulls out slowly and gently. [line break][speech style of M]'If you keep being a good locked up [if the player is male]sissy[otherwise]baby[end if] for Nanny, you'll get more where that came from.'[roman type][line break]";
...
To say DiaperChangeStart of (M - matron):
	say "[BigNameDesc of M], with seemingly little physical exertion, picks you up and plops you down on the giant changing table. She smoothly and efficiently binds your wrists and ankles tightly with the inbuilt straps.".
	...
	if the class of the player is adult baby:
		...
		if N > 0 and a random number between 1 and 4 is 1:
			while N > 0:
				...
				say "[BigNameDesc of M] takes away your [V]!";
				decrease N by 1;
			...
			if baby's bottle is not held:
				...
				now the doses of baby's bottle is the max-doses of baby's bottle;
		...
		if there is a worn necklace and a random number between 1 and 10 is 1 and the favour of M < 13:
			...
			say "[BigNameDesc of M] notices your [C]!  [line break][speech style of M]'Where did you get this, baby?!  This is my jewellery!  Bad [if the player is male]sissy[otherwise]girl[end if], this isn't yours to play with!  It's precious and delicate. I'll be taking it back now.'[roman type][line break]She pulls it off from your neck!";
...
To say ForcefeedStartFlav of (M - matron):
	say "[BigNameDesc of M], with seemingly little physical exertion, picks you up and plops you down on the giant high chair. She pulls the tray into place in front of you and locks it shut, preventing you from escaping. [if the number of worn bibs is 0]She gets a large pink bib with an image of a cartoon animal on it, and fastens it around your neck. [end if][line break][speech style of M]'Time for your dindins!'[roman type][line break]".
...
To compute talk option (N - 1) to (M - matron):
...
To say UnfriendlyResponse of (M - matron):
...
To say DominantResponse of (M - matron):
...
To say ToDominantGreeting of (M - matron):
