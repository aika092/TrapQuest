...
This is the spawn initial robomatron rule:
	if the number of alive robomatrons is 0 and diaper quest is 1:
		let M be a random robomatron;
...
To compute motion of (M - a robomatron):
	do nothing.

To compute (M - a robomatron) seeking (D - a direction):
	do nothing.

To compute fleeing of (M - a robomatron):
	do nothing.
...
Definition: a robomatron (called M) is willing to spank gently:
	decide no.

To decide which number is the spanking length of (M - a robomatron):
	decide on a random number between 3 and 4.

Definition: a robomatron (called M) is willing to deliver enemas:
	decide yes.

Definition: a robomatron (called M) is eager to enhance the spanking:
	decide yes.

To say SpankingMercyRejectionFlav of (M - a robomatron):
...
To say SpankingAfterFlav of (M - a robomatron):
	say "After this [he of M] seems satisfied, lowers you to the floor gently and allows you to crawl away.".

To decide which number is the enema quarts of (M - a robomatron):
	decide on 4.

To say EnemaDeclarationFlav of (M - a robomatron):
...
To compute damage of (M - a robomatron):
	if the health of M > 0:
		if M is unfriendly:
			if the health of M > the maxhealth of M / 2, say "The [M] doesn't pay any attention to the damage!";
			otherwise say "[one of]The [M]'s movement seems slightly more erratic now, as if her joints have suffered some proper damage.[or]The [M] is forced to slightly recoil![stopping]";
		otherwise:
			anger M;
			now M is interested;
