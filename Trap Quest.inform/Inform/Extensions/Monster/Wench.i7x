...
A wench is a kind of monster. A wench is usually intelligent. A wench is usually female. A wench has a number called suffocation. The wench has a number called guard-obedience. The leftover-type of a wench is usually 102.
...
To compute (M - a monster) stomping (N - a wench):
	if M is in the location of the player:
		say "With lightning speed [NameDesc of M] grabs the [N] and starts fucking her ass furiously. She screams in both pleasure and terror and he starts coming inside her within seconds. Her belly expands in front of her as he fills her up. After she is full, he tosses her to the ground, where she promptly faints.";
...
To compute perception of (M - a wench):
	now M is interested;
	say "[BigNameDesc of M] notices you[if the player is sluttily dressed][one of] even though [he of M] is in no better state herself[or][or][or][or][cycling][end if].";
	...
	otherwise if M is objectifying the player:
		...
		now the sex-length of M is 2;
		if image cutscenes is 1, display figure of wench interact 9;
	otherwise if M is not horny-wench and there are held desirable knickers and the number of knickers retained by M is 0:
		...
		otherwise:
			...
			anger M;
			now the sex-length of M is 2;
			say "[speech style of M]'... that [ShortDesc of K]... where did you get it? Give it to me!'[roman type][line break]";
			if image cutscenes is 1, display figure of wench interact 5;
	otherwise:
...
To display interaction of (M - a wench):
	if M is horny-wench:
		if M is interested, display figure of wench interact 8;
		otherwise display figure of wench interact 3;
	otherwise if M is guardian:
		display figure of wench interact 7;
	otherwise if M is ally:
		display figure of wench interact 1;
	otherwise if M is buddy:
		display figure of wench interact 6;
	...
	otherwise if M is uninterested:
		display figure of wench interact 10.
...
To compute (M - a wench) protecting against (X - a monster):
	if X is male or the player is monster stuck:
		...
		if image cutscenes is 1, display figure of wench interact 11;
	...
	otherwise:
		...
		if image cutscenes is 1, display figure of wench interact 12;
		distract M.
...
Report dropping clothing in the presence of a wench:
	...
	if M is not horny-wench and the noun is desirable and the number of knickers retained by M is 0:
...
To compute kneeling reaction of (M - a wench):
	if the guard-obedience of M is 0:
		say "[one of][BigNameDesc of M] walks over to you, momentarily stopping the flow of cum out of her pussy by cupping a hand to her entrance.[or][BigNameDesc of M] gently fingers herself as she strides over to you, grinning as [semen] slowly leaks through her fingers.[or]A grin spreads across [NameDesc of M]'s plastered face as she cups her hand to her [vagina], stopping the flow of [semen] down her legs.[or][BigNameDesc of M] grins, pointing at the thick glob of [semen] rolling down her leg, and then at you.[or][BigNameDesc of M] looms over you, her cum dripping pussy level with your head. She leers down at you, grinning.[in random order]";
...
To compute facial sex of (M - a wench):
	...
	if the body soreness of the player + the suffocation of M >= 17:
		say "[BigNameDesc of M] continues to torture you with her crotch. You really need to breathe soon!  You are so exhausted there is nothing you can do, you struggle weakly but her ass cheeks and [vagina] and the unending stream of [semen] are completely preventing you from breathing. You pass out.";
		[if image cutscenes is 1, display figure of wench cutscene 7;] [Using this as penetration image instead]
...
To get facial penetration image for (M - a wench):
	if image cutscenes is 1, display figure of wench cutscene 7.

To compute (M - a wench) entering mouth:[TODO: alter for gangbangs]
	if milking is 1:
		do nothing;
	otherwise:
		...
		if presented-orifice is face or the player is subdued or M is friendly-fucking or the player is live fucked, now B is 0; [If the player consents, she flips you automatically]
		...
		if R > B:
			...
			if bukkake fetish is 1:
				...
				CumFaceUp 1;
			say "[one of]Her hold on your neck and head is too strong, you'll have to choose between either [bold type]resist[roman type]ing and risk it taking longer until you can breathe or [bold type]submit[roman type]ing by licking her out, swallowing the [semen] dripping from her pussy and hoping she [if the oral sex addiction of the player < 3]finishes[otherwise if the oral sex addiction of the player < 7]climaxes[otherwise]cums over your face[end if] quickly[or]Once again, you won't be able to breathe until she's finished with you[stopping]!";
...
To compute unique death of (M - a wench):
	...
	if the health of M <= 0:
		if white-bodice is off-stage:
			...
			say "[big he of M] [if the sleep of M is 0]drops to the ground, dead[otherwise]stops breathing[end if]. [big his of M] body disappears, leaving the [white-bodice] she was wearing.";
			compute autotaking white-bodice;
		otherwise:
			say "[big he of M] [if the sleep of M is 0]drops to the ground, dead[otherwise]stops breathing[end if]. [big his of M] body disappears.";
			loot M.
...
To compute failed dominance punishment of (M - a wench):
	if the player is female or the appearance of the player >= 8 and M is not objectifying the player and the times-dominated of M < a random number between 1 and 3:
		...
		otherwise if the size of penis > 0:
			...
			otherwise if the player consents:
				if the charisma of the player > a random number between 1 and 3:
					...
					arouse 3000;
				otherwise:
					say "[line break][variable custom style]'[one of]Please let me cum!'[or]Please, I need to orgasm!'[or]Please, I want to cum!'[at random][roman type][line break] You blurt out. [BigNameDesc of M] laughs derisively as [he of M] removes [his of M] thumb from your [sissy-penis], forcing you to orgasm just with your ass.";
...
To say DominanceSuccess of (M - a wench):[TODO: eventual penetration]
	if the size of penis > 8:
		if player is a bit horny:
			say "[BigNameDesc of M] takes one look at your rock hard [ShortDesc of penis] and tries to scramble away, but you grab [him of M] by the hair and force [him of M] down onto [his of M] knees. [line break][first custom style]'Where you goin['] bitch? I thought you were all about drinking cum.'[roman type][line break][big he of M] [if M is horny-wench]shakes her head, but immediately forms an O-shape with her lips, drooling a bit as her tongue lolls out over her lower lip[otherwise]shakes her head, but after a moment she blushes slightly and slowly opens her mouth anyway[end if]. You revel in the sound of [his of M] gagging as you shove it in, brutally fucking [his of M] face until [he of M] begins to do it all by herself. [big he of M] skilfully strokes the underside of your shaft with [his of M] tongue, further streaking her makeup with fresh tears as [he of M] repeatedly impales [his of M] face on your rock hard junk.";
			Dignify 20;[Extra dignity when you save up]
		otherwise:
			say "You force [NameDesc of M] to [his of M] knees. She [if the times-dominated of M > 0]gulps, staring at your [manly-penis] as [he of M] slowly opens [his of M] mouth. A faint blush colours [his of M] cheeks[otherwise]tentatively opens [his of M] mouth and begins to fondle her breasts, eyeing your [manly-penis] with apprehension. Her expression grows even more uncertain[end if] as your [ShortDesc of penis] reaches its full size, and [his of M] eyes get wide as your giant boner nears [his of M] lips. [big he of M]'s silent for a moment, [if M is horny-wench]before grabbing it with both hands, eagerly guiding[otherwise]then slowly reaches out with one hand, guiding[end if] it into [his of M] mouth. You feel [his of M] tongue move skilfully over the underside of your shaft, gently lubricating it as it reaches deeper and deeper down [his of M] throat. [big he of M] does such it with such precision and control that you can't bear the thought of interrupting, [run paragraph on]";
		if the player is desperate to pee and watersports fetish is 1:
			say " so you go for the next best thing and immediately start peeing directly into [his of M] belly! [big his of M] eyes widen at first, but after a moment [he of M] just accepts it, staring up at you submissively as you empty every last drop of your hot [urine] into [his of M] tummy. You shove [his of M] off and quickly nut all over [his of M] face when you're finished.";
			now the bladder of the player is 0;
			Dignify 65;
		otherwise:
			say " [if the oral sex addiction of the player > 3 and the throatskill of the player is 0]in fact, you're actually a little jealous of [his of M] technique[otherwise if the oral sex addiction of the player > 3]in fact, you can't help feeling [he of M] might actually be even better than you are[otherwise]but you're still getting the feeling that [he of M] isn't really making use of [his of M] full potential[end if]. You make sure [he of M] has to put [his of M] skills to good use, making sure [he of M] has at least 7 inches in [his of M] mouth at all times, and holding [his of M] wrists above [his of M] head so [he of M] can't cheat by using [his of M] hands. [if M is horny-wench][big he of M] seems to love the treatment, so [he of M] has a lot of trouble focusing on the exercise[otherwise]It's clear [he of M]'s no master at doing things without the training wheels on[end if], but by the time your load spurts down [his of M] throat, [he of M]'s learned a whole lot about how to properly [one of]service[or]gag on[or]please[at random] your [ShortDesc of penis]. You shove [his of M] off without waiting for [his of M] to finish, allowing the rest of your [semen] to splatter liberally across [his of M] face and breasts.";
			Dignify 50;
		orgasm;
		say AfterDominationComment 5 of M;
	otherwise if the size of penis > 6:
		say "You force [NameDesc of M] to [his of M] knees, brandishing your fully erect [ShortDesc of penis]. [line break][variable custom style]'[if the player is desperate to pee and watersports fetish is 1]So, are you ready to drink my piss?[otherwise]So, do you feel like sucking my [manly-penis][end if]?'[roman type][line break][big he of M] [if the times-dominated of M > 0 or M is horny-wench]nods eagerly[otherwise]gulps, avoiding eye contact as [he of M] answers with a slow nod[end if], gripping the base of your shaft with one hand and lightly stroking it as [he of M] drags [his of M] tongue up your length. You didn[']t really come here for foreplay though, and just as you're ready to 'nudge' [him of M] into beginning the main event, [his of M] lips wrap themselves around the end of your [manly-penis] and immediately glide down your spit-coated shaft. [big he of M] furrows [his of M] eyebrows as your tip brushes the back of [his of M] throat, bracing [his of M] hands on your thighs as [he of M] forces herself down even further.";
		if the player is desperate to pee and watersports fetish is 1:
			say "You decide it's the perfect time to give [him of M] the drink you promised, and hold [him of M] down as you begin pissing straight down [his of M] throat. [big he of M] splutters a bit, but otherwise seems to accept it, staring up at you submissively until your bladder has been completely and thoroughly emptied into [his of M] belly, and you allow [him of M] to pull up.";
			now the bladder of the player is 0;
			Dignify 40;
		otherwise:
			say "Pleasure explodes up your shaft as tiny movements of [his of M] tongue push you to the brink of orgasm in no-time-flat. You barely manage to grunt before you go over the edge, involuntarily bucking your hips as you unload straight down [his of M] throat. [if the throatskill of the player is 1][big he of M]'s clearly had a lot more practice than you have, because [end if][big he of M]'s able to hold the position for the full duration of your orgasm, sitting up gracefully as your softening [manly-penis] to slips out of [his of M] mouth.";
			orgasm;
			Dignify 30;
		say AfterDominationComment 1 of M;
	otherwise if the size of penis > 3:
		say "You force [NameDesc of M] to [his of M] knees. [big he of M] [if the player is a bit horny]seems to accept [his of M] role without much difficulty, gripping the base with one hand as [his of M] wraps her lips around the head[otherwise]stares at your hardening [ShortDesc of penis] for a moment before slowly nodding, gripping the base with one hand as [he of M] wraps [his of M] lips around the head[end if].";
		if the player is desperate to pee and watersports fetish is 1:
	...
	otherwise if the size of penis > 0:
		...
		if the player is desperate to pee and watersports fetish is 1:
			...
			Dignify 15;
		otherwise:
			say "[speech style of M]Wow, now I feel even worse...just do it on my chest alright?[roman type] [big he of M] pushes [his of M] breasts together, looking up at you with a deadpan expression as you eagerly pump your with your fingers [if the player is a bit horny]desperately hard [end if][ShortDesc of penis]. You wish [he of M] were more enthusiastic, but it just feels so good, and your time in this game has left you a lot more sensitive than you were before. It only takes a few short moments for you to emit a feminine grunt and spurt your [load] all over [his of M] chest.";
...
To oral dominate (M - a wench):[TODO: update]
	...
	if the player is male:
		say "You force [NameDesc of M] onto her back, sitting on her face. [if the bimbo of the player < 12][line break][first custom style]'[one of]No offence meant[or]Nothing personal[at random], I promise.'  [otherwise][line break][second custom style]'Let's see how you like it!'  [end if][roman type][line break]Pulling your asscheeks apart, you position your [asshole] over her mouth, scooting back slightly to cut off airflow to [his of M] nose. [big he of M] whines petulantly, trying to squirm away, failing because [he of M] is weak from the preceding fight. Reluctantly, [he of M] opens [his of M] mouth and begins to massage your sphincter with [his of M] tongue. ";
		if the semen volume of belly > 0:
			say "You sigh in pleasure, the sensations causing your anal muscles to loosen and push [semen] out onto [his of M] face. [BigNameDesc of M] immediately starts enjoying [him of M]self a lot more, grabbing your thighs and beginning to eagerly thrust [him of M] tongue in and out of your [asshole]. It's fun having someone clean out your [asshole] when everyone else here seems bent on violating it, but eventually you grow bored and get up.";
		...
		otherwise:
			say "You take the opportunity to play with [his of M] breasts, pinching and rubbing [his of M] pink nipples until they are tight and red. However, it doesn't really feel all that great for you, so you eventually grow bored and climb to your feet.";
			say "[AfterDominationComment 1 of M]";
	otherwise:
		if the player is desperate to pee and watersports fetish is 1:
...
To say PresentFriendlyAcceptanceFlav of (M - a wench):
	...
	if image cutscenes is 1, display figure of wench interact 2.
...
To compute (M - a wench) considering (T - a vessel):
	if M is unfriendly:
		...
		if image cutscenes is 1, display figure of wench interact 13;
	...
	otherwise if the blue-balls of M > 0 and T is open topped:
		...
		if image cutscenes is 1, display figure of wench interact 8;
		now the doses of T is 2;
		now the fill-colour of T is creamy;
	otherwise:
		say "[BigNameDesc of M] shakes her head no.[line break][speech style of M]'What would I even fill it up with?'[roman type]";
		if image cutscenes is 1, display figure of wench interact 10.
...
To say MonsterTakeFlav of (M - a wench) to (T - a vessel):
	say "[BigNameDesc of M] takes the [T] from you [if the doses of T > 0], dumping it out before she[otherwise]and[end if] turns away, quietly moaning to herself for a moment or so before handing returning the [T], full of creamy [semen].".
...
To say RepeatResponse of (M - a wench):
	...
	if there is knickers retained by M:
		...
		if image cutscenes is 1, display figure of wench interact 14;
	otherwise:
		say "[speech style of M]'[one of]I hope you don't mind me staying close. I'm scared.'[or]Do you mind if I stay close? I'm scared.'[or]I used to be an adventurer like you, but then I took a cumshot to the ass.'[as decreasingly likely outcomes][roman type][line break]";
		if image cutscenes is 1, display figure of wench interact 12.
...
To say midDominanceResponse of (M - a wench):
	...
	otherwise:
		...
		if image cutscenes is 1, display figure of wench interact 3.
...
To say WhoAnswer of (M - a wench):
	say "[speech style of M]'My name's [if M is creampied wench]Amy[otherwise if M is lost wench]Katie[otherwise if M is kidnapped wench]Stephanie[otherwise]Mary[end if], pleased to meet you, I guess.'[roman type][line break]";
	if image cutscenes is 1, display figure of wench interact 7.
...
To compute friendly drink of (M - a wench):
	...
	if image cutscenes is 1, display figure of wench interact 1.
