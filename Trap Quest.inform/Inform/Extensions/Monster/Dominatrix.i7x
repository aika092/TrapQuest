...
To say MonsterComment of (M - dominatrix):
	if the strap-on girth of M < 4:
		...
		otherwise if the player is female and the bimbo of the player < 14:
			say "[variable custom style][one of]Why does that strap-on look so appetizing?[or]Why do I feel like I should do anything [he of M] says?[or]I feel safe when I[']m near [him of M], but scared at the same time.[in random order]";
		...
		otherwise:
			say "[second custom style][one of]I[']ll give anyone a blowjob, even a girl![or]I prefer guys, but that strap-on looks almost as good as the real thing![or]All [manly-penis]s make [if the size of penis < 1]horny[otherwise]hard[end if], even fake ones![if the size of penis < 1]Between us, there[']s only one penis, but [he of M] probably deserves it way more![otherwise if the size of penis < 8]So that[']s what a real penis looks like. Well, it's plastic, but it's certainly realer than mine. I[']m the girl and [he of M][']s the man! Tee-hee![otherwise]My penis is bigger than [his of M] strap-on, but [hers of M] matters more, since it feels better when I cum with my asshole.[end if][in random order]";
...
Check going when dominatrix is in the location of the player:
	if the disappointment of dominatrix > 1, now the strap-on girth of dominatrix is the disappointment of dominatrix. [Increase the strap-on size if appropriate]
...
To suggestFucker (F - a body part) for (M - dominatrix):
	...
	otherwise if F is face:
		say "Do you want to see how [he of M] reacts to having [his of M] strap-on used on [him of M]?";
		if the player is consenting:
			now player-fucker is F;
...
To say DominanceSuccess of (M - dominatrix):
	say "You successfully force [NameDesc of M] to [his of M] knees, holding [him of M] down as you rip off [his of M] strap-on, leaving [his of M] crotch bare. [big he of M] [if the times-dominated of M < 3]stares at you silently for a moment before cocking an eyebrow and spreading [his of M] legs. [line break][speech style of M]'Alright. Here[']s your chance.'[otherwise]looks away from you, waiting a moment before submissively spreading [his of M] legs. [line break][speech style of M]'I'm ready.'[end if][roman type][line break]";
...
To say AfterDominationComment (N - a number) of (M - dominatrix):
	...
	if N is 2, say "[first custom style]'[one of]I admit, you-... you[']re not... you aren[']t submissive. Sorry. [if the player is male]Sir[otherwise]Ma'am[end if].[or]I won[']t try to make you my slave any more. [if the player is male]S-sir[otherwise]Ma'am[end if].'[at random][roman type][line break]";
