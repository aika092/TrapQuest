...
A patron is a kind of monster.  A patron is usually intelligent.  A patron is usually dark skinned.  A patron is usually male.  A patron is usually raunchy.  A patron is usually father material.  A patron is usually willing to do anal.  A patron is usually willing to do vaginal.  A patron is usually willing to do oral.  A patron is usually willing to do titfucks.  A patron can be pissed off.  A patron is usually not pissed off.  The difficulty of a patron is usually 3.  A patron has an object called wanking-target.
...
To say PatronDesc:
	...
	if item described is inexperienced patron, say "This particular guy looks like he's in his early twenties, and is very over-eager.[line break][if item described is penetrating a body part and the sex addiction of the player < 7][first custom style]Well I hope that means he'll cum quickly...[otherwise if item described is penetrating a body part][variable custom style]He's clearly nowhere near as experienced as me![otherwise][variable custom style]Maybe he's a virgin?[end if][roman type][line break]";
	if item described is gross patron, say "This particular man is quite overweight, his hairy belly hangs out from his shirt and his gross [if item described is hairy patron]hairy[otherwise]flabby[end if] chest is fully visible.[line break][if item described is penetrating a body part and the sex addiction of the player < 7][first custom style]Oh my god, he's so gross![otherwise if the sex addiction of the player < 7][variable custom style]I do NOT want to go anywhere near his dick![otherwise if the humiliation of the player < 27500][variable custom style]He's pretty gross.[otherwise][variable custom style]He's disgusting, I'm disgusting.  We make a perfect match.[end if][roman type][line break]";
	if item described is experienced patron:
		if diaper quest is 1, say "This man is wearing a smart suit and has quite a serious mature face.  [if the sex addiction of the player < 7][first custom style][one of]He isn't thinking that this is going to get sexual, is he?[or]Not again![stopping][otherwise if the sex addiction of the player > 13][second custom style][one of]I hope he gives obedient babies their cummies![or]Yay, Daddy's home![stopping][otherwise][variable custom style]Something tells me he's going to be in charge...[end if][roman type][line break]";
		otherwise say "This particular man is wearing a smart suit and he doesn't stare at you like you might expect, seeming only mildly interested.  You guess that he's been with a lot of whores in his time, and just wants a body to use to get his rocks off.[line break][if the sex addiction of the player < 7][first custom style]Trust me, I'm not interested in you either, buddy.[otherwise if the humiliation of the player > 27500][variable custom style]Well he's right, I'm just an object to be used.[otherwise if the bimbo of the player > 10][second custom style]He certainly knows how to frustrate an attention whore like me![otherwise][variable custom style]The least he could do is at least appreciate my beauty![end if][roman type][line break]";
	if item described is dickhead patron, say "This particular man stares at you with narrow eyes.  It looks like he's both aroused and disgusted by you.[line break][if the sex addiction of the player < 7][first custom style]I'm just disgusted.[otherwise if the humiliation of the player > 30500][variable custom style]I'm disgusted by me too.[otherwise if the sex addiction of the player > 12][second custom style]I'm just aroused![otherwise][variable custom style]The feeling is mutual![end if][roman type][line break]".
...
To compute labour to (M - a patron):
	...
	if the player is in the hotel:
		...
		say "[PregFlav]A human head pushes its way out of your cunt, agonisingly slowly and painfully.  Finally the head has passed through, and after a final push at the shoulders, the birthing is complete.  You pick your child up off the floor and start to cradle it in your arms.  The [father] appears almost as if on cue!  [PregFlav of the father]Severing the umbilical cord, he pulls the baby from your hands, wraps it in a soft blanket and then carries it away.  [variable custom style][if the bimbo of the player < 8]That is definitely for the best.[otherwise if the bimbo of the player < 14]I guess I don't really have anywhere to raises a baby in here, anyway.[otherwise]Oh good, that means I can busy myself with finding a new man to breed me again![end if][roman type][line break]";
	otherwise:
		Delay Labour;
...
To compute diaper mess reaction of (M - a patron):
	if diaper quest is 1:
		say "The [M] [one of]visibly recoils[or]cries out with shock[or]steps back in horror[or]yells with surprise[in random order].  [if there is a barrier in the location of the player][speech style of M]'[one of]Yuck!  You're even more gross than I expected from a baby slave.  Do you have no shame?!'[or]What the hell?  This wasn't supposed to happen.  Can you really not control yourself?!'[in random order][roman type][end if][line break]";
	otherwise:
		say "The [M] [one of]visibly recoils[or]cries out with shock[or]steps back in horror[or]yells with surprise[in random order].  [if there is a barrier in the location of the player][speech style of M]'[one of]Woah, this is not what I wanted!!!  Too far!'[or]What the hell?  This isn't what I'm paying for!  Oh god, that smell!'[or]What the fuck are you doing?! Yuck, stop!!!'[or]Is this some kind of prank?  That stinks!'[or]Nope, sorry, way too far for me.  I'm out.'[or]Nope, nope nope nope.  This is not my scene.  No thank you!'[in random order][roman type][end if][line break]";
...
To say CondomRejectFlav of (M - a dickhead patron):
	say "[if chosen-orifice of M is vagina and pregnancy fetish + the pregnancy of the player is 1][speech style of M]'Didn[']t you hear me? I[']m gonna get you pregnant.'[roman type] Looks like there[']s no way out of a creampie...[otherwise][speech style of M]'No thanks.'[roman type] Looks like [he of M]'s going in bare...".
...
To compute kneeling reaction of (M - a patron):
	if the number of patrons in the location of the player is 1, say "The [M] grins.  [speech style of M]'[one of]That's more like it.'[or]That's what I thought.'[or]Now you're going to get it.'[or]Time to get what I came here for.'[or]Where you belong.'[in random order][roman type][line break]";
	humiliate 75.
...
To say waitingflav of (M - a patron):
	if the player is not able to speak:
		say "[one of][variable custom style][muffled sounds][roman type][line break][or]You look up at the [if the number of patrons in the location of the player is 1][M][otherwise]men[end if] silently.[at random]";
	otherwise if diaper quest is 1:
		if the diaper addiction of the player < 7:
			say "[one of]You stare up at the [M], still in shock[or]You sigh, having lost any real hope of negotiation being successful[stopping].  [if the player is able to speak][variable custom style]'[one of]I'm sorry, WHAT?  What is this place??  Do you think I'm some kind of diaper fetishist?'[or]There's been some kind of mistake, I'm not the baby you're looking for!'[or]Look, can't we talk about this first?  There must be another way...'[stopping][roman type][line break]Unsurprisingly, he completely ignores you.[end if]";
		otherwise if the diaper addiction of the player < 12:
			say "You [one of]cough nervously[or]squirm uncomfortably[or]shuffle awkwardly[in random order]. [variable custom style][if the player is able to speak]'[one of]Please be gentle, okay?'[or]You're not going to be mean to me, are you?'[or]If I play along, do you promise to be kind?'[or]Alright, if you're paying, let's do this.'[in random order][otherwise][gag sounds][end if][roman type][line break].";
		otherwise:
			say "You [one of]mewl playfully[or]grin gleefully[or]bounce up and down on the bed[or]squeal with delight[in random order]. [variable custom style][if the player is able to speak]'[one of]Ooh yay, this should be fun!'[or]'Ooh yay, are we gonna play a fun game?'[or]I'm your little baby, do what you want with me!  Tee-hee!'[in random order][otherwise][gag sounds][end if][roman type][line break]";
	otherwise if the number of patrons in the location of the player is 1:
		if the sex addiction of the player < 7:
			say "[one of]You stare up at the [M], still in shock.  [or]You sigh, having lost any real hope of negotiation being successful.  [stopping][variable custom style]'[one of]I'm sorry, WHAT?  What is this place??  Do you think I'm some kind of prostitute?'[or]There's been some kind of mistake, I'm not the whore you're looking for!'[or]Look, can't we talk about this first?  There must be another way...'[stopping][roman type][line break]Unsurprisingly, he completely ignores you.";
		otherwise if the sex addiction of the player < 12:
			if M is dickhead patron:
				say "[one of]You glare at the rude man.  [or]You harrumph loudly.  [at random][variable custom style]'[one of]Look here, bozo.  You fuck me, you pay me.  No funny business, okay?'[or]Hey, treat me nicely, please?  I'm just trying to make an honest living here.'[or]Fuck you, jackass.  I'll [if the chosen-orifice of M is face]suck you off[otherwise]fuck you[end if], but only because it's literally my job.'[in random order][roman type][line break]";
			otherwise if M is gross patron:
				say "[one of]You hesitantly reply.  [or]You look at the barrier, and then back at the man.  [at random][variable custom style]'[one of]I guess I'll do it for one ring.'[or]I guess I should just be glad that you'll pay me at all...'[or]I guess I'll let you decide how much it's worth.  Just please don't hurt me?'[in random order][roman type][line break]";
			otherwise:
				say "[one of]You stare up at the [M] nervously.  [or]You look at the barrier, and then back at the man.  [at random][variable custom style]'[one of]You're going to be gentle with me, right?'[or]You're going to pay me after, right?'[or]But I don't have any condoms...  Okay, just this once.'[in random order][roman type][line break]";
		otherwise:
			if M is dickhead patron:
				say "[one of]You mewl playfully.  [or]You stick your tongue out playfully.  [at random][second custom style]'[one of]Whatever you say goes, Mister!'[or]Hey, I'm just a cheap whore.  You get to treat me however you like, Mister!'[or]Ooh yes sir, I am good at following orders, sir. [if the chosen-orifice of M is face]Pleease can I have permission to suck on your tasty fuckstick and swallow your yummy [semen]?'[otherwise]Please use me like your own personal fucksleeve for as long as you like~'[end if][in random order][roman type][line break]";
			otherwise if M is inexperienced patron:
				say "[one of]You smile gently.  [or]You giggle playfully.  [at random][second custom style]'[one of]Slow down kiddo, we've got all the time in the world.  There's no rush, you can use me as you want.'[or]Whatever you say, kiddo.  But take your time!  You've got all the time you want until you cum.'[in random order][roman type][line break]";
			otherwise if M is gross patron:
				say "[one of]You smile gently.  [or]You giggle playfully.  [at random][second custom style]'[one of]If I didn't need the money, I'd be doing this for free.'[or]I don't like to think of it as payment, so much as a tip for a job well done.'[in random order][roman type][line break]";
			otherwise:
				say "[one of]You stare up at the [M] with a warm inviting smile.  [or][if the chosen-orifice of M is face]You obediently get onto your knees and drool at the sight of his [manly-penis][otherwise]You obediently turn around and push your [HipDesc] into the air[end if].  [at random][variable custom style]'[one of]Your wish is my command, honey.'[or]Honey, I was born to serve handsome [manly-penis]s like yours.'[or]I love a man that knows what he wants.'[in random order][roman type][line break]";
	otherwise if the sex addiction of the player < 6 + the number of patrons in the location of the player: [Multiple patrons!]
		say "[one of]You stare up at the men, in shock that there's [if the number of patrons in the location of the player is 2]more than one[otherwise]so many[end if].  [or]You yelp in fright, not expecting to have to service [if the number of patrons in the location of the player is 2]two at a time[otherwise]quite so many men at once[end if].  [stopping][variable custom style]'[one of]Look, there's been a mistake, I've never serviced more than one patron at a time before...'[or]Oh come on, this is just too much for one person to handle, surely?!'[or]This cannot be happening...'[stopping][roman type][line break]Unsurprisingly, they completely ignore you.";
	otherwise if the sex addiction of the player < 14:
		say "[one of]You stare up at the men nervously.  [or]You are paralysed by a mixture of excitement and hesitation.  [at random][variable custom style]'[one of]Hey, how do you guys feel about agreeing on a safe word?  No?  Oh well, it was worth a try...'[or]You're each going to pay me, right?  I'm not doing extra for free.'[or]I don't usually take this many customers at once you know.  But just this once, I'll let it slide, as long as you're careful with me.'[line break]As if I really had a choice...[in random order][roman type][line break]";
	otherwise:
		say "[one of]You stare up at the men and beckon them invitingly with your finger.  [or]You obediently turn around and push your [HipDesc] into the air, spanking it once for good measure.  [at random][variable custom style]'[one of]All aboard, gentlemen.  Dinner is served.'[or]If I can [if the body soreness of the player < 8]walk[otherwise]even move[end if] by the time you're [if the number of patrons in the location of the player > 2]all[otherwise]both[end if]done, I'm going to be unimpressed.'[or]Yes, yes, this is more like it!  If I don't have [manly-penis]s in [if the player is male]both my holes[otherwise if the number of patrons in the location of the player is 2]two of my holes[otherwise]all my holes[end if] within the next five seconds I'm gonna be disappointed.  Well come on, hurry up!!!'[in random order][roman type][line break]".
...
To say LandingTaunt of (M - a patron):
	say "The [M] smirks.  [speech style of M]'[one of]Haha, at last![or]I've never seen such a ridiculous sight in my life.[in random order]'[roman type][line break]";
	humiliate 50.
...
To compute SelectionWaiting of (M - a patron):
	...
	otherwise:
		...
		say "[if the semen addiction of the player < 6][variable custom style][one of]Yuck![or]Gross![purely at random][otherwise if the semen addiction of the player < 10][variable custom style]Eww![otherwise][second custom style]Ooh, it's so warm![end if][roman type][line break]";
...
To say SoftSpankingFlav of (M - a patron):
	say "The [M]'s resolve weakens, and then he delivers just a few weak blows to your [buttcheeks] with [his of M] hand.  [if there is a worn diaper]Your [random worn diaper] softens the blows even further, so that they just feel like gentle pats.  [end if][speech style of M]'Well I guess I'll let you off this time, but that was really boring.'[roman type][line break]The [M] lets you go.";
	now M is pissed off;
	compute payment of M.
