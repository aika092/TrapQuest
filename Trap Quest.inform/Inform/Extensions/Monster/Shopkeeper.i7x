...
To compute perception of (M - shopkeeper):
	...
	otherwise if the class of the player is vixen:
		...
		anger M;
	otherwise if the refactory-period of M > 0:
...
To say PenetrationFlav of (M - shopkeeper) in (F - vagina):
	...
	otherwise if the virgin of the player is 1 and M is virginity taking:
...
To say PrepTaunt of (M - shopkeeper) in (F - a fuckhole):
	...
	otherwise:
		say "[line break][speech style of M]'[one of]Let's have fun, alright?'[or]I've been waiting for this!'[or]I've got a nut with your name on it!'[or]Lets have some of that bomb-ass sex.'[at random][roman type][line break]";
...
To compute unique death of (M - shopkeeper):
	let X be a random off-stage accessory;
	...
	say "[big he of M] drops to [his of M] knees, defeated. [big he of M] then fades into nothingness, leaving behind a [if X is strapon-panties][X] and a [end if]few valuable gems.";
	loot M;
	loot M;
	loot M;
...
To say BimboSeduced of (M - shopkeeper):
	if M is interested and the class of the player is cheerleader and diaper quest is 0:
		say "[BigNameDesc of M] is [if M is unfriendly]even more [end if]turned on by your incessant chattering![if M is friendly][line break][speech style of M]Oh baby, I had hoped you'd say something like that. Come here, [boy of the player], I'll show you how to have a great time...[roman type][line break][end if]";[The chatterbox is his dream girl!]
	otherwise:
...
To compute annoyance of (M - shopkeeper):
	...
	otherwise:
		say "[speech style of M]'[big please], I have a shop to run, I can't stand around all day answering questions.'[roman type][line break]".
...
To anal dominate (M - shopkeeper):
	...
	otherwise:[blacked]
		...
		say "It's hard not to want to be on the bottom, but you[']re still in control. Definitely. You lower yourself slowly, [if Ofit < -1]hissing[otherwise if Ofit < 2]groaning[otherwise]moaning[end if] as you cram inch upon inch of big, [if interracial fetish is 1]black[otherwise]hard[end if], shaft deeper and deeper into your hole. [big he of M] seems to notice that you're distracted, slipping one hand out from under [his of M] body, and casually moving it up your outer thigh, giving your [AssDesc] a firm swat as [he of M] begins to thrust. You want to stop [him of M], but it's like [his of M] fingertips are made of electricity; Every movement of [his of M] hands sends a shiver of excitement up your spine, and it doesn't really help that your hips have already begun to match [his of M] pace. You [if there is a worn chastity cage or the size of penis is 0]want desperately to touch yourself, pushing out your chest[otherwise if the size of penis < 4 and the size of penis > 0]find yourself rubbing your [ShortDesc of penis] with one hand and pushing out your chest[otherwise]find yourself rapidly stroking your [ShortDesc of penis] with both hands[end if] as [he of M] sensually [if the largeness of breasts < 5]palms your [ShortDesc of Breasts][otherwise]plays with your [ShortDesc of Breasts][end if]. You feel [him of M] stiffening slightly inside you, and as [his of M] breathing runs ragged, yours does too. You realize [he of M]'s going to cum. [big his of M] [LongDickDesc of M] is about to explode! [line break][variable custom style]'[one of]Yes! Cum inside me! Please cum inside me[or]Yes! Omigosh yes[or]Yes! Yes! Cum inside me[at random]![roman type][line break]";
		now player-fucking is DOMINANT-SHAMEFUL;
		say "[big he of M] groans, grabbing your hips as [he of M] cums with all the force of an avalanche. Your [if there is a worn chastity cage]cage hangs uselessly[otherwise if the size of penis is 0]heart thumps in your chest[otherwise if the size of penis < 4][sissy-penis] hangs uselessly over [his of M] belly[otherwise][player-penis] rests uselessly on [his of M] belly[end if] as [if M is wrapped]the condom captures wave after wave of warm [semen][otherwise][he of M] plants wave after wave of [semen] deep inside your [asshole][end if]. [run paragraph on]";
		if face is actually occupied:[TODO: check that incorporates BBC addiction]
		...
		otherwise:
			...
			if M is wrapped:
				say "[line break][second custom style]I have to taste it.[roman type][line break] You pull off [him of M] slowly, savouring the feeling of the warm, stretchy bulge inside you as you kneel down in front of [him of M], [if C is 1]half-realizing you didn't get to cum[otherwise if the size of penis < 4]rubbing your [sissy-penis] with two fingers[otherwise]pumping away with one hand[end if] as you peel the latex sheath from [his of M] softening [manly-penis] and eagerly dump its contents into your mouth. [if C is 1]You can feel every nerve in your body tingling as you swallow it all in one gulp[otherwise]The feeling of fresh [semen] hitting your tongue is just enough to put you over the edge, and girlcum trickles down your thighs as you swallow it all in one gulp[end if].";
				if C is 0, orgasm;
...
To compute successful dominance of (M - shopkeeper):
	...
	DominateUp M;
	DifficultyUp M by 3;
	if the location of M is Dungeon41, say "[line break][BigNameDesc of M] pops a blue pill and hops to [his of M] feet, full of energy.";
	otherwise say "[line break][BigNameDesc of M] slinks away as soon as [he of M] has the strength.";
	if player-fucking is DOMINANT-DOMINANT:
		say "You feel more dominant!";
	otherwise if player-fucking is DOMINANT-DOMINANT:
		say "You feel a little more dominant.";
	otherwise:
		say "That didn't feel very dominant...";

To say AfterDominationComment (N - a number) of (M - shopkeeper):
	if N is 1:
		say "[speech style of M]'[one of]Damn, good move, I guess. Choose what you want quick, alright?'[or]At least you didn't steal anything. Choose what you want quickly.'[or]Alright. This isn't an extended discount, so be quick.'[at random][roman type][line break]";
	otherwise if N is 2:
