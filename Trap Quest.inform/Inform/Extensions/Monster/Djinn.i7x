...
Definition: djinn (called M) is father material:
	decide yes.
...
Definition: djinn (called M) is human:
	decide yes.
...
To say SummoningFlav of (M - djinn):
...
To say DiaperReaction of (M - djinn):
	...
	if the humiliation of the player < 15000, say "[one of]You are so ashamed you wish the ground would just open up and swallow you. [or][stopping]You turn bright red and can't bring yourself to look [NameDesc of M] in the eye.".
...
To say SexSubmissionFlav of (M - djinn): [this is customized because I rewrote the scene so the player is floating a couple feet off the ground.]
	...
	otherwise:
		say "[one of]You allow [NameDesc of M] to violate you as much as he likes. He's in charge![or]You relax and enjoy the feeling of [NameDesc of M]'s rock hard [if interracial fetish is 1]giant black [end if][DickDesc of M].[at random]".

To say SexResistFlav of (M - djinn):
	say "[one of]You hopelessly flail against [NameDesc of M]'s grip, doing everything you can to resist the invasion.[or]You seem to be being held still by some kind of magical force, leaving you with no real way of resisting, but you still try![or]You flail around and make as much noise as you can, but no matter how much you try, you can't do anything about [NameDesc of M]'s powerful pistoning.[or]You fight against the [if interracial fetish is 1 and M is penetrating a fuckhole]pleasure that his big black [DickDesc of M] is sending through your sensitive hole[otherwise][M][end if] but it's no use, you can't do anything to stop the action![in random order]".
...
To get PenetrationImage of (M - djinn) in (F - vagina):
...
To compute post climax effect of (M - djinn) in (F - vagina):
	if M is unwrapped and pregnancy fetish is 1 and the pregnancy of the player is 0:
		say "You immediately feel the [if the player is queen of spades]superior virility of [his of M] nubian sperm[otherwise]magical sperm[end if] barrelling down your tubes and straight to your undefended egg[if inhuman pregnancy > 0]s[end if]. A strange tingle shoots through you from deep inside, kind of in your belly, almost, and you feel certain you're now pregnant with [his of M] child[if inhuman pregnancy > 0]ren[end if]![line break]";
...
Definition: djinn (called M) is willing to let go:
	decide no.
...
This is the djinn punishes diaper rule:
	...
	if the player is diaper focused:
		...
		if R > 1 and D is transformable:
			say "[BigNameDesc of M] points at your [D], and a bolt of magic lightning erupts from his finger!  Your [D] starts to shimmer...";
			...
			otherwise:
				...
				satisfy M;
		otherwise:
			say "[BigNameDesc of M] points at your [D], and a bolt of magic lightning erupts from his finger!  You watch as a ball of [urine] appears in mid-air in front of you, floating like a bubble, before flattening into a golden stream which flies straight for you!  It rushes up through the leg-holes of your [D] and splatters against you!";
			PissSoak 10 on D;
...
Definition: djinn (called M) is willing to spank gently:
	decide no.
...
Definition: djinn (called M) is willing to confiscate:
	decide yes.
...
To say UnfriendlyResponse of (M - djinn):
...
To say DominantResponse of (M - djinn):
...
To say midDominanceResponse of (M - djinn):
...
To say AsDominantResponse of (M - djinn):
...
To compute annoyance of (M - djinn):
	if M is uninterested:
		say "The djinn doesn't seem to realize you are talking to [him of M].[line break]";
	otherwise if M is unfriendly:
