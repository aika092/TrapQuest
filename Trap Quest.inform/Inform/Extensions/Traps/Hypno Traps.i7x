...
To say hypno content of (Y - a fucktoy hypno trap):
	say "a swirling hypnotic pattern, with short clips flashing constantly of [if the player is male]transsexual [end if]porn stars getting [if the player is not possessing a vagina]anally destroyed[otherwise if pregnancy fetish is 1]vaginally bred[otherwise]double penetrated[end if] by huge [manly-penis]s".
...
To compute specific hypno effect of (Y - a fucktoy hypno trap):
	...
	if the player is not possessing a vagina, AnalSexAddictUp 1;
...
To compute specific hypno effect of (Y - a diaper hypno trap):
	if the player is a june 2019 diaper donator, commence animation of DiaperHypnoAnimation;
