...
To say ExamineDesc of (T - a hypno trap):
	say "A monitor revealed from within a hidden compartment in a wall. [if T is expired]The screen has now turned off[otherwise]On the screen is [hypno content of T]. It's difficult to look away once you start watching it[end if].".
...
To trigger (Y - a hypno trap):
	...
	say "A section of the wall in front of you slides away, and behind it is a magic crystal display. Suddenly, the crystal starts playing [hypno content of Y] as if it was a giant TV screen!";
...
To compute hypno effect of (Y - a hypno trap):
	...
	otherwise:
		if woman-player is in the location of the player and (the woman-status of woman-player is not 91 or the mechanic-scene of woman-player > 3) and woman-player is friendly and woman-player is awake:
			...
			otherwise:
				compute woman hypno solo progression;
