...
To compute condomCatch of (V - a vine) in (F - a fuckhole):
	...
	if the player is consenting:
		...
		if R <= 3 or (the player is prone and R <= 6):
			if the player is prone, say "[one of]You manage to slip free just long enough to pull out a condom, only to be caught and pinned a moment later. [if V is penetrating F]Another[otherwise]A[end if] vine shoots out of the soil and [if the TrapNo of V > 1]squeezes[otherwise]plunges[end if] its way into your [variable F].[or]You manage to free your hands long enough to catch the incoming vine with a condom, but as you're unrolling it, a second vine shoots out of the soil and into your unprotected [variable F]![or]No matter how hard you try, you can't free your hands long enough to pull out a condom. You [if sex addiction of the player > 7]moan[otherwise]grunt[end if] as [if V is penetrating F]another[otherwise]a[end if] vine [if the TrapNo of V > 1]squeezes[otherwise]shoots[end if][or]You slip one hand free and quickly pull out a condom. You try to capture the vine with a condom, but it's too fast! You hand is knocked out of the way as it [if the TrapNo of V > 1]squeezes[otherwise]slams directly[end if] into your [variable F].[at random]";
