...
Definition: mirror soiling is acceptable:
	if (the bladder of the player > 2 and there is worn knickers) or (rectum > 3 and there is a worn total protection diaper and diaper lover >= 4) or the player is diaper focused or the player is an adult baby, decide yes;
	decide no.
Definition: mirror soiling is one-turn:
	decide no.

To compute resolution of (X - mirror soiling):
	if the player is diaper focused or the player is an adult baby:
