...
Definition: A thing is insertable:
	...
	if it is a bed post, decide yes;
...
To compute periodic effect of (C - an expansion clothing):
	...
	if the size of C < 10 and a random number between 1 and R is 1:
		increase the size of C by 1;
		let F be a random fuckhole penetrated by C;
		if the size of C > the openness of F, Ruin F;
		say "Your [printed name of C] drastically increases in size![if the size of C > the openness of F] You feel your [variable F] stretching painfully to accommodate the enlarged intruder[otherwise] You feel [clothing-material of C] pressing much tighter against the inside of your [variable F][end if].";
		if the player is male, PenisUp 1.[expansion clothing will slowly increase penis size for male players]
...
To decide which number is the grip of (I - an insertable thing):
	...
	if I is a bed post, decide on 999;
...
To say MonsterOfferAcceptFlav of (M - a wrestler) to (T - a plug):
	say "[speech style of M]'Ooh, fine, you've tempted me.'[roman type][line break]The [M] unzips the crotch of her latex outfit, and with a lewd, shameless face, works the plug into her asshole, before zipping her outfit back up.  [speech style of M]'Tada!'[roman type][line break]".
