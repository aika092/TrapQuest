Buttocksless Pants by Trousers begins here.

buttockless pants is a kind of trousers. A buttockless pants is usually butt-windowed. The armour of buttockless pants is 10. The printed name of buttockless pants is usually "[TQlink of item described][clothing-title-before][clothing-material of item described] buttockless pants[clothing-title-after][TQxlink of item described][verb-desc of item described]". The text-shortcut of buttockless pants is "bp". Figure of yellow buttockless pants is the file "buttocklesspants1.png". Figure of blue buttockless pants is the file "buttocklesspants2.png". The soak-limit of buttockless pants is usually 17.

To say ShortDesc of (Y - a  buttockless pants):
	say "pair of lewd buttockless pants".

blue buttockless pants is a kind of buttockless pants. There is 1 denim blue buttockless pants. yellow buttockless pants is a kind of buttockless pants. There is 1 leather yellow buttockless pants. 

The text-shortcut of yellow buttockless pants is "lbp".

To decide which figure-name is clothing-image of (H - a blue buttockless pants):
	decide on figure of blue buttockless pants.

To say ClothingDesc of (B - a blue buttockless pants):
	say "A pair of navy denim jeans with the buttocks cut out. It provides modesty to the crotch area[if diaper focus is 1] but allows everyone to see what underwear or nappy you're wearing[otherwise if the outrage of B is too humiliating] but is still extremely humiliating to wear[end if].".

To decide which number is the initial outrage of (B - a blue buttockless pants):
	decide on 5.

To decide which object is the potential-upgrade-target of (C - a blue buttockless pants):
	decide on a random off-stage yellow buttockless pants.


To decide which figure-name is clothing-image of (H - a yellow buttockless pants):
	decide on figure of yellow buttockless pants.

To say ClothingDesc of (B - a yellow buttockless pants):
	say "A pair of yellow pants with the buttocks cut out. It provides modesty to the crotch area[if diaper focus is 1] but allows everyone to see what underwear or nappy you're wearing[otherwise if the outrage of B is too humiliating] but is still extremely humiliating to wear[end if].".


To decide which number is the initial outrage of (B - a yellow buttockless pants):
	decide on 6.

To decide which object is the potential-upgrade-target of (C - a yellow buttockless pants):
	if the number of worn knickers is 0:
		if the player is male, decide on a random off-stage pouch panties;
		otherwise decide on a random off-stage crotch rope;
	otherwise if the number of worn skirted clothing is 0:
		decide on a random off-stage naughty skirt;
	decide on nothing.

Buttocksless Pants ends here.

