...
+!]
Check rubbing:
	unless the noun is magic lamp, try cleaning the noun with instead.
...
Rule for supplying a missing second noun while cleaning: 
	let RagFound be 0;
	if the semen taste addiction of the player >= 8 and the player is not overly full:
		now the second noun is arms;
	unless the humiliation of the player > 28000 and the semen addiction of the player > 11 and the second noun is arms:
		repeat with C running through clothing carried by the player:
			if the semen-soak of C + the urine-soak of C + the milk-soak of C < the soak-limit of C and C is appropriate for cleaning and RagFound is 0:
				say "(with the [printed name of C])[line break]";
				now the second noun is C;
				now RagFound is 1;
	if RagFound is 0 and the semen taste addiction of the player >= 8:
		now the second noun is arms;
	if RagFound is 0 and the noun is body part:
		if the second noun is nothing, say "You don't appear to have any appropriate items you could clean yourself with.";
	otherwise if RagFound is 0:
