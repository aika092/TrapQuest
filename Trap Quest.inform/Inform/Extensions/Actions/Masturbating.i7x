...
Definition: yourself is fapping-blocked:
	if there is a worn chastity cage or there is a worn restricting research airhancer, decide yes;
...
Monsters are MUCH more likely to go for the ass if the player is fingering theirself.
...
Check masturbating:
	...
	if the buttskill of the player is 1:
		if the player is fapping-blocked:
			now asswank is 1;	
			if auto is 0:
				say "You can only use your ass to masturbate right now right now[if the player is not anal flexible and the player is upright], and you'll have to get on your knees first[end if]. Is that ok? [yesnolink]";
	...
	if asswank is 0 and the buttskill of the player is 1:
		say "Use your ass to masturbate? [if the player is not anal flexible and the player is upright]You'll have to get on your knees first.[end if][yesnolink]";
...
Carry out masturbating:
		...
		repeat with O running through insertable objects carried by the player:
			if the girth of O < the openness of vagina + 3:
				...
				if the player consents:
					now object-choice is O;
					break;
	if asswank is 0 and the player is able to wank and auto is 0 and object-choice is nothing:
		repeat with W running through vibe-wands carried by the player:
			say "Would you like to use the [printed name of W] to masturbate with? [yesnolink]";
			if the player consents:
	...
	if asswank is 1:[its harder to cum with your ass]
		increase N by 2;
	...
	if debugmode > 0, say "[line break]After modifications, N is [N] (1 or less is instant).";
	if N <= 1:
		if asswank is 1, say InstantAnalMasturbationFlav of (object-choice) in (C);
		otherwise say InstantMasturbationFlav of (object-choice) in (C);[Flavour here]
	...
	otherwise:
		...
		if asswank is 1, say StartAnalMasturbationFlav of (object-choice) in (C);[Flavour here]
		otherwise say StartMasturbationFlav of (object-choice) in (C);
		...
		while N > 1:
			arouse 3000;
			decrease N by 1;
			if asswank is 1, say OngoingAnalMasturbationFlav of (object-choice) in (C);
			otherwise say OngoingMasturbationFlav of (object-choice) in (C);[Flavour here]
			compute extra turn;
	...
	otherwise:
		if asswank is 1, say ClimaxAnalMasturbationFlav of (object-choice) in (C);
		otherwise say ClimaxMasturbationFlav of (object-choice) in (C);[Flavour here]
		...
		otherwise:
			...
			if the player is female and object-choice is not an insertable object, heal vagina times 1;
	...
	if object-choice is an insertable object:[we always want to make sure we dislodge the sex toy]
		dislodge object-choice;
		[if asswank is 1, now object-choice is not penetrating asshole;
		otherwise now object-choice is not penetrating vagina;]
	now asswank is 0.

To say InstantMasturbationFlav of (O - an object) in (C - an object):[We're only doing an introduction here, the rest is handled by the climaxmasturbationflav function; sex addiction here should be raw sex addiction]
	let M be a random live thing penetrating a body part;
	let A be the raw sex addiction of the player;
	if the player is male:
		let R be a random number between 1 and 3;
		if R is 1:
			say "You [if O is vibe-wand]turn on the [ShortDesc of O] and grind your [ShortDesc of penis] against it[otherwise if C is clothing]stroke your [ShortDesc of penis] through your [ShortDesc of C][otherwise]eagerly begin to stroke your [ShortDesc of penis][end if], [if M is a thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if]. [if A < 5]Its almost embarrassing how much it turns you on[otherwise if A < 10]Its a little surprising how much it turns you on[otherwise]It makes you uncontrollably turned on[end if], as the pleasure rapidly builds upon itself, you realize you're already about to cum.";
		otherwise if R is 2:
			say "You [if O is vibe-wand]turn on the [ShortDesc of O] and[end if] begin to rub your [ShortDesc of penis] [if C is clothing]through your [ShortDesc of C][end if], closing your eyes as you [if M is a thing]focus on [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if]. Nothing more than a moment passes, [if A < 5]and you are immediately ashamed to[otherwise if A < 10]and you tell yourself to be ashamed to[otherwise]and you feel your muscles tightening in anticipation as you[end if] realize that you're already going to cum.";
		otherwise if R is 3:
			say "You [if O is vibe-wand]switch on [ShortDesc of O] and rub the vibrating end against your [ShortDesc of penis][otherwise if C is clothing]eagerly begin to stroke your [ShortDesc of penis] through your [ShortDesc of C][otherwise]eagerly begin to stroke your [ShortDesc of penis][end if]. [if A < 5 and M is a thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realize you're already going to cum.";
	otherwise:
		if O is insertable object:
			say "[one of]You eagerly begin to fuck yourself with the [printed name of O], [if M is a thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if]. [if A < 5]Its almost embarrassing how much it turns you on[otherwise if A < 10]Its a little surprising how much it turns you on[otherwise]It makes you uncontrollably turned on[end if], and as the heat of new arousal lights up your body, you realize you're already about to cum[or]You slide the [printed name of O] into your [vagina], and begin to vigorously fuck yourself to [if M is a thing]the feeling of [NameDesc of M][otherwise]the thought of [PlayerFantasy][end if]. Nothing more than a moment passes, [if A < 5]and you are immediately ashamed to[otherwise if A < 10]and you tell yourself to be ashamed to[otherwise]and you feel your muscles tightening in anticipation as you[end if] realize that you're about to cum.[or]You sink the [printed name of O] into your [vagina] and begin fucking yourself with abandon. [if A < 5 and M is a thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realize you're already going to cum.[at random]";
		otherwise if O is vibe-wand:
			say "[one of]You turn on the [printed name of O] and grind yourself against it, [if M is a thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if]. [if A < 5]Its a little embarrassing how much it turns you on[otherwise if A < 10]It gets you even more turned on[otherwise]It makes you uncontrollably turned on[end if], and as the heat of arousal lights up your body, you realize you're already going to cum.[or]You switch on the [printed name of O] and hold it against your clit, closing your eyes as you [if M is a thing]focus on [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if]. Already, the rapidly intensifying pleasure seems to head toward a peak, and you realize [if A < 5]with shame [otherwise if A < 10] [otherwise]with anticipation [end if]that you're about to cum.[or]You turn on the [printed name of O], holding the vibrating end against your clit. [if A < 5 and M is a thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realize you're already going to cum.[at random]";
		otherwise if C is clothing:
			say "[one of]You eagerly begin to rub your clit through your [ShortDesc of C], [if M is a thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if]. [if A < 5]Its a little embarrassing how much it turns you on[otherwise if A < 10]It gets you even more turned on[otherwise]It makes you uncontrollably turned on[end if], and as much as the sensation is dampened by the [clothing-material of C], you realize you're already going to cum.[or]You begin to rub yourself through your [ShortDesc of C], closing your eyes as you [if M is a thing]focus on [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if]. Even through the fabric, the pleasure rapidly intensifies, and you realize [if A < 5]with shame [otherwise if A < 10] [otherwise]with anticipation [end if]that you're about to cum.[or]You touch yourself through the [ShortDesc of C], teasing your clit through the fabric. [if A < 5 and M is a thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realize you're already going to cum.[at random]";
		otherwise:
			say "[one of]You eagerly begin to rub your clit[unless vagina is actually occupied] and finger your [vagina][end if], [if M is a thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if]. [if A < 5]Its a little embarrassing how much it turns you on[otherwise if A < 10]It gets you even more turned on[otherwise]It makes you uncontrollably turned on[end if], and as the rush of new arousal passes through your body, you realize you're already going to cum.[or][if vagina is actually occupied]You eagerly diddle your clit[otherwise]You eagerly rub your clit whilst fingering your [vagina][end if], closing your eyes as you [if M is a thing]focus on [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if]. Already, the rapidly intensifying pleasure seems to head toward a peak, and you realize [if A < 5]with shame [otherwise if A < 10] [otherwise]with anticipation [end if]that you're about to cum.[or]You eagerly begin to touch yourself, [if vagina is actually occupied]delicately, but enthusiastically teasing your sensitive clit[otherwise]rubbing your clit and fingering your [vagina][end if].[if A < 5 and M is a thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realize you're already going to cum.[at random]";
	say "[line break]";

To say InstantAnalMasturbationFlav of (O - an object) in (C - an object):
	let A be the anal sex addiction of the player;
	let M be a random live thing penetrating a body part;
	let R be a random number between 1 and 3;
	if R is 1:
		say "You [if O is insertable object]ease the [printed name of O] into your [asshole][otherwise]ease your fingers into your [asshole][end if], [if A < 5]cheeks burning with shame[otherwise if A < 10]panting with excitement[otherwise]cooing with unabashed excitement[end if] as you begin to fuck yourself to the thought of [PlayerFantasy]. Nothing more than a moment passes, [if A < 5]and you are immediately ashamed to[otherwise if A < 10]and you tell yourself to be ashamed to[otherwise]and you feel your muscles tightening in anticipation as you[end if] realize that you're about to cum.";
	otherwise if R is 2:
		say "[if O is insertable object]The [printed name of O][otherwise]Your fingers[end if] send ripples of pleasure through your body as you push them into your [if the soreness of asshole > 3]sensitive [end if][asshole], eliciting a hiss of pleasure as you [if the player is male and O is nothing]begin to delicately stroke your prostate[otherwise if the player is male]begin to grind the [printed name of O] against your prostate[otherwise if O is insertable object]gingerly push the [printed name of O] in and out[otherwise]gingerly push your fingers in and out[end if]. [if A < 5 and M is a thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realize you're already going to cum.";
	otherwise if R is 3:
		say "You eagerly slide your [if O is insertable object][printed name of O][otherwise]fingers[end if] into your [asshole], closing your eyes and beginning to fuck yourself [if M is a thing]to the feeling of [NameDesc of M][otherwise]as your mind fills with thoughts of [PlayerFantasy][end if]. [if A < 5]You can't help feeling ashamed at how much it turns you on[otherwise if A < 9]You can't believe how much it turns you on[otherwise]It gets you unbelievably turned on[end if], and as the rush of new arousal passes through your body, you realize that you're about to cum.";
	say "[line break]";

To say StartMasturbationFlav of (O - an object) in (C - an object):
...
To say StartAnalMasturbationFlav of (O - an object) in (C - an object):
	let R be a random number from 1 to 3;
	if O is insertable object:
		if R is 1:
		...
		otherwise:
			say "You [if player is not horny]sigh softly[otherwise if player is horny]moan quietly[otherwise]shudder with pleasure[end if] as you reach between your legs and sink the [printed name of O] into your [asshole], and slowly beginning to fuck yourself.";
	otherwise:
		if R is 1:
			say "You reach betwene your legs and slip your fingers into your [asshole]. You [if player is not horny]sigh softly[otherwise if player is horny]moan quietly[otherwise]shudder with pleasure[end if] as you begin to gently push them in and out.";
		otherwise if R is 2:
...
To say ClimaxMasturbationFlav of (O - an object) in (C - an object):[note, this appears even when the player has an instant orgasm]
	let M be a random live thing penetrating face;
	let X be a random live thing penetrating a fuckhole;
	if the player is male:
		say "You [if C is clothing or O is vibe-wand]rub[otherwise]stroke[end if] yourself faster and faster, [if the bimbo of the player < 7]groaning[otherwise]moaning[end if] [if M is a thing]into [NameDesc of M][otherwise]with excitement and pleasure[end if] as you finally go over the edge.";[the rest is handled by the orgasm function]
	otherwise:
		if O is insertable object:
			say "You [if M is a thing]moan into [NameDesc of M][otherwise if the raw sex addiction of the player < 5]grit your teeth, panting[otherwise if the raw sex addiction of the player < 10]purse your lips, moaning[otherwise]can't help but form your lips into a large, circular 'O', cooing[end if] with pleasure as the feeling of hard [if O is golden phallus]gold[otherwise]plastic[end if] sliding in and out of your [vagina] [if the player is very horny]finally [end if]pushes you over the edge. Your [vagina] [one of]clamps down[or]spasms desperately[or]clenches[at random] around the [printed name of O] as it [SquirtDesc], powerful waves of pleasure overwhelming your senses as [one of]orgasm wracks your body[or]orgasm crashes through your body[or]orgasm crashes over you[at random].";
		otherwise if O is vibe-wand:
			say "You [if M is a thing and M is penetrating face]moan into [NameDesc of M][otherwise if the raw sex addiction of the player < 5]grit your teeth, panting[otherwise if the raw sex addiction of the player < 10]purse your lips, moaning[otherwise]can't help but form your lips into a large, circular 'O', cooing[end if] with pleasure as the intense clitoral stimulation from your [printed name of O] [if the player is very horny]finally [end if]pushes you over the edge. [if X is a thing]Your [vagina] [one of]clamps down[or]spasms desperately[or]clenches[at random] around [NameDesc of X] as it[otherwise if the player is upright]Your knees go weak as your muscles tense then relax as your [vagina][otherwise]Your jaw goes slack as your muscles tense then relax as your [vagina][end if] [SquirtDesc], powerful waves of pleasure overwhelming your senses as [one of]orgasm wracks your body[or]orgasm crashes through your body[or]orgasm crashes over you[at random].";
		otherwise if C is clothing:
		...
		otherwise:
			say "You find yourself moaning as the [if the raw sex addiction of the player < 6]slow building pleasure finally approaches[otherwise if the raw sex addiction of the player < 13]the buildup of pleasure approaches[otherwise]the fast building pleasure races toward[end if] a climax, inner muscles [if X is a thing][one of]clamping down[or]clenching[at random] around [NameDesc of X][otherwise]tensing and relaxing around your fingers[end if] as you go over the edge. [if M is a thing]Your moans are muffled by [NameDesc of M][otherwise if the humiliation of the player < HUMILIATION-PROUD]You can't even begin to stifle your moans[otherwise]Uncontrolled moans stream out of your mouth[end if] as your body erupts with pleasure, [if the openness of vagina < 4]girlcum dribbling down your leg[otherwise]girlcum spraying out between your fingers[end if] as shivers of pleasure run up and down your spine.";
		say "[line break]";

To say ClimaxAnalMasturbationFlav of (O - an object) in (C - an object):
	let M be a random live thing penetrating face;
	let X be a random live thing penetrating a fuckhole;
	if the player is male:[ejaculation is handled by the orgasm function]
		if O is insertable object:
			say "You [if M is a thing]moan through [NameDesc of M][otherwise if the anal sex addiction of the player < 5]grit your teeth, grunting[otherwise if the anal sex addiction of the player < 10]purse your lips, moaning[otherwise]can't help but form your lips into a large, circular 'O', cooing[end if] with pleasure as the feeling of hard [if O is golden phallus]gold[otherwise]plastic[end if] [one of]punching your prostate[or]ramming your sensitive prostate[or]grinding against your prostate[at random] [if the player is very horny]finally [end if]pushes you over the edge. You feel your anal muscles rapidly tensing and relaxing, intensifying the feeling of the [printed name of O] inside you as waves of pleasure ripple through your body.";
		otherwise:
			say "You find yourself moaning as the [if the raw sex addiction of the player < 6]slow building pleasure finally approaches[otherwise if the raw sex addiction of the player < 13]the buildup of pleasure approaches[otherwise]the fast building pleasure races toward[end if] a climax, inner muscles [if X is a thing][one of]clamping down[or]clenching[at random] around [NameDesc of X][otherwise]tensing and relaxing around your fingers[end if] as you go over the edge. [if M is a thing]Your moans are muffled by [NameDesc of M][otherwise if the humiliation of the player < HUMILIATION-PROUD]You can't even begin to stifle your moans[otherwise]Uncontrolled moans stream out of your mouth[end if] as your prostate tightens up, anal muscles rapidly tensing and relaxing as shivers of pleasure run up and down your spine.";
	otherwise:
		if O is insertable object:
			let V be a random thing penetrating vagina;
			say "You [if M is a thing]moan through [NameDesc of M][otherwise if the anal sex addiction of the player < 5]grit your teeth, panting[otherwise if the anal sex addiction of the player < 10]purse your lips, moaning[otherwise]can't help but form your lips into a large, circular 'O', cooing[end if] with pleasure as the feeling of hard [if O is golden phallus]gold[otherwise]plastic[end if] sliding in and out of your [asshole] [if the player is very horny]finally [end if]pushes you over the edge. Your [vagina] [if V is a thing][one of]clamps down[or]spasms desperately[or]clenches[at random] around the [printed name of V][otherwise]clenches around nothing[end if] as it [SquirtDesc], powerful waves of pleasure overwhelming your senses as [one of]orgasm wracks your body[or]orgasm crashes through your body[or]orgasm crashes over you[at random].";
		otherwise:
			let V be a random thing penetrating vagina;
			say "You find yourself moaning as the [if the anal sex addiction of the player < 6]slow building pleasure finally approaches[otherwise if the anal sex addiction of the player < 13]the buildup of pleasure approaches[otherwise]the fast building pleasure races toward[end if] a climax, anal muscles [if V is a thing][one of]clamping down[or]clenching[at random] around [NameDesc of V][otherwise]tensing and relaxing around your fingers[end if] as you go over the edge. [if M is a thing]Your moans are muffled by [NameDesc of M][otherwise if the humiliation of the player < HUMILIATION-PROUD]You can't even begin to stifle your moans[otherwise]Uncontrolled moans stream out of your mouth[end if] as your body erupts with pleasure, [if the openness of vagina < 4]girlcum dribbling down your leg[otherwise]girlcum spraying out between your legs[end if] as shivers of pleasure run up and down your spine.";

To say OngoingMasturbationFlav of (O - an object) in (C - an object):
	let M be a random live thing penetrating a body part;
	if the player is male:
		if O is vibe-wand:
			say "[one of]You shudder with pleasure as your [printed name of O] vibrates against your [player-penis], [if M is a thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if].[or]You half register the sticky wetness of your precum as you use your [printed name of O] to smear it up and down your shaft, [if M is a thing]totally asorbed in pleasuring [NameDesc of M] even more[otherwise]too enthralled by the thought of [PlayerFantasy][end if].[or]The [printed name of O] provides a unique sensation as you [if the size of penis < 3]tenderly[otherwise if the humiliation of the player < HUMILIATION-MODEST + 4000]abashedly[otherwise]eagerly[end if] rub it against your [player-penis], [if M is a thing]allowing [NameDesc of M] to use you[otherwise]fantasizing about [PlayerFantasy][end if].[or]You continue to stimulate your [player-penis] with your [printed name of O], [if the bimbo of the player < 8]trying to ignore[otherwise]relishing in[end if] its unmistakable sex toy hum as you [if M is a thing]close your eyes and focus on how [NameDesc of M] feels[otherwise]fantasize about [PlayerFantasy][end if].[or]You lose yourself to pleasure, [if the bimbo of the player < 4]grunting[otherwise if the bimbo of the player < 8]groaning[otherwise if the bimbo of the player < 12]moaning[otherwise if the bimbo of the player < 15]cooing[otherwise]shamelessly moaning[end if] as you vigorously rub the [printed name of O] against your [player-penis].[or]You use your [printed name of O] to pin your [player-penis] against your body, precum rolling off your shaft as you [if M is a thing]use your body to pleasure both yourself and [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if].[at random]";
		otherwise if C is clothing:
			say "[one of]You shudder with a mixture of [if the bimbo of the player < 8]shame[otherwise]frustration[end if] and pleasure as you massage the tip of your [player-penis] with your [ShortDesc of C], [if M is a thing]your focus on [NameDesc of M][otherwise]fantasizing about [PlayerFantasy][end if].[or]You half register the sticky wetness of your precum soaking into your [ShortDesc of C] as you stroke your [player-penis] through the fabric, too enthralled by [if M is a thing][NameDesc of M][otherwise]the thought of [PlayerFantasy][end if].[or]The [ShortDesc of C] provides a unique sensation as you [if the size of penis < 3]tenderly[otherwise if the humiliation of the player < HUMILIATION-MODEST + 4000]abashedly[otherwise]eagerly[end if] rub it against your [player-penis], [if M is a thing]as you feel [NameDesc of M] use you[otherwise]fantasizing about [PlayerFantasy][end if].[or]You continue to clamp and squeeze your [player-penis] through your [ShortDesc of C], [if the bimbo of the player < 8]trying to ignore[otherwise]relishing in[end if] its tightness against your skin as you [if M is a thing]close your eyes and focus on the feel of [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if].[or][CummingInPantiesFantasy of C][or]You lose yourself to pleasure, [if the bimbo of the player < 4]grunting[otherwise if the bimbo of the player < 8]groaning[otherwise if the bimbo of the player < 12]moaning[otherwise if the bimbo of the player < 15]cooing[otherwise]shamelessly moaning[end if] as you vigorously rub the bunched up [clothing-material of C] of your [ShortDesc of C] against your [player-penis].[or][CooingMasturbation of C][or][DirtyThoughtsMasturbation of C][or]Your [player-penis] strains against your [ShortDesc of C], leaking precum into the [clothing-material of C] as you [if M is a thing]use your body to pleasure both yourself and [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if].[at random]";
		otherwise:
			say "[one of]You shudder with pleasure, continuing to work your [player-penis] as you [if M is a thing]use your body to pleasure both yourself and [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if].[or]You eagerly stroke your [player-penis], [if the bimbo of the player < 4]grunting[otherwise if the bimbo of the player < 8]groaning[otherwise if the bimbo of the player < 12]moaning[otherwise if the bimbo of the player < 15]cooing[otherwise]shamelessly moaning[end if] as your mind runs wild with increasingly [if M is a thing]submissive[otherwise]dirty[end if] thoughts.[or]You hold your [player-penis] [if the size of penis < 2]as tightly as you can with only two fingers, eagerly stroking yourself to[otherwise]tightly as you stroke yourself to[end if] the [if M is a thing]rhythm of [NameDesc of M]'s thrusts[otherwise]thought of [PlayerFantasy][end if].[or]Your [player-penis] drools precum freely onto the floor, [if the size of penis > 4]bobbing[otherwise]jiggling[end if] as you passionately [if the size of penis < 4]tug[otherwise]stroke[end if] it[unless M is a thing] to fantasies of [PlayerFantasy][otherwise]to the feeling of [NameDesc of M][end if].[or]Your surroundings fall away as the sensations begin to overwhelm you, clouding your thoughts and coloring your voice as you [if the size of penis < 4]tug[otherwise]stroke[end if] away.[or]You find it increasingly hard to control the sound of your voice as you [if the size of penis < 4]tug[otherwise]stroke[end if] away,  heart racing and thoughts slowly trickling away as orgasm closes in.[or]Your precum leaks into your hand, lubricating your [player-penis], and adding a strangely satisfying [']shlick['] to every movement of your hands. [if the humiliation of the player > HUMILIATION-PROUD + 2000]It's not loud enough for someone outside the room to hear it, but [line break][second custom style]you wish it was[roman type][line break].[otherwise if M is a thing]It isn't loud, but you know [NameDesc of M] knows what it is[end if][or]You murmur as you [if M is a thing]focus on [NameDesc of M][otherwise]fantasize about [PlayerFantasy][end if], breathing faster and faster as the pleasure begins to build up in your [player-penis].[or]You tremble with pleasure, your hand moving at a pace all its own as it relentlessly pumps your [player-penis] closer and closer to orgasm.[at random]";
	otherwise:
...
To say OngoingAnalMasturbationFlav of (O - an object) in (C - an object):
	...
	otherwise:
		...
		otherwise:
			let P be a random worn nipple covering clothing;
			say "[one of]You fantasize about [PlayerFantasy]. You feel your fingers [if the urine volume of belly > 0]the lukewarm urine trapped inside[otherwise if the total egg fill of belly > 0]bumping the eggs incubating in[otherwise if the semen volume of belly > the milk volume of belly + 3]moving through the thick creampie deep inside [otherwise if the milk volume of belly + the semen volume of belly > 0]moving through the creamy filling planted in [otherwise]as they stroke the inside of [end if] your [asshole] as your mind continues racing.[or]You [if the player is male]eagerly stroke your prostate[otherwise]eagerly fuck your [asshole][end if], moaning [if the humiliation of the player < HUMILIATION-MODEST]quietly[otherwise]loudly[end if] as your mind turns to thoughts of [PlayerFantasy].[or]You slowly move your fingers in and out of your [asshole], [if P is clothing]rubbing your [BreastDesc] through your [printed name of P][otherwise]tweaking your nipples[end if] as you fantasize about [PlayerFantasy].[or]You fuck yourself with your own fingers, [if the bimbo of the player < 5]shuddering[otherwise if the bimbo of the player < 12]trembling[otherwise]rolling your eyes back[end if][if the semen volume of vagina < 1] at how inexorably pleasurable the Nintendolls managed to make it feel[otherwise if the semen volume of belly < 4] at the feeling of the slimy [semen] trapped in your [vagina][otherwise] as thick, creamy [semen] coats your fingers and rolls down your arm.[end if][or]You continue your passionate fingering session, [if the bimbo of the player < 7]trying and failing to stifle your shameful moans[otherwise if the bimbo of the player < 13]half-heartedly trying to stifle your passionate moans[otherwise]allowing shameless, wanton moans to stream out of your mouth[end if] as you fantasize about [PlayerFantasy].[at random]";
