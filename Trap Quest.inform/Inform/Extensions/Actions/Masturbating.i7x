...
Definition: yourself is able to automatically masturbate:
	...
	if yourself is able to masturbate:
		...
		decide yes;
	if yourself is able to anally masturbate:
		now auto is saved-auto;
		decide yes;
	now auto is saved-auto;
	decide no.
...
The sex doll can't wank rule is listed last in the anal masturbation restriction rules.
...
The too slutty to wank rule is listed last in the anal masturbation restriction rules.
...
This is the research restriction blocks masturbation rule:
	if there is a worn restricting research airhancer and the player is male:
...
This is the bound hands blocks anal masturbation rule:
	if the player is wrist bound or there is a thing wrangling the player:
		if auto is 0, say "You can't finger your [asshole] while your wrists are bound!";
		rule fails.
The bound hands blocks anal masturbation rule is listed last in the anal masturbation restriction rules.
...
The already wanking rule is listed first in the anal masturbation restriction rules.
...
The embarrassing masturbation rule is listed last in the anal masturbation restriction rules.
...
This is the clothing blocks anal masturbation rule:
	if the number of bottom level ass protection clothing > 0:
		if auto is 0, say "You would need to displace your [printed name of a random bottom level ass protection clothing] first.";
		rule fails.
...
This is the boner use blocks masturbation rule:
	if there is a live thing penetrating penis:
		let T be a random thing penetrating penis;
		if auto is 0, say "Your [player-penis] is already being used by [FuckerDesc of T]!";
		rule fails.
The boner use blocks masturbation rule is listed in the masturbation restriction rules.

This is the anal penetration blocks anal masturbation rule:
	if asshole is actually occupied:
		let T be a random thing penetrating asshole;
		if auto is 0, say "Your [asshole] is already occupied by [FuckerDesc of T]!";
		rule fails.
...
This is the stiffness blocks anal masturbation rule:
	if the player is upright and the player is not anal flexible:
		if auto is 0, say "You aren't flexible enough to finger your [asshole] without getting on your knees.";
		rule fails.
...
+!]
Check masturbating:[TODO: after gender update, penis masturbation and vaginal masturbation will need to be separate]
	if the player is not able to masturbate:
		if the buttskill of the player is 0 or the player is not able to anally masturbate, do nothing instead;
		otherwise now asswank is 1;
	if (the wanktime of the player > 0 or lock-it tattoo is worn) and auto is 0 and the number of worn diapers is 0 and the number of on-stage chastity cages is 0:
		say "You hesitate, having a sneaking suspicion that [masturbating][if diaper quest is 1] while not wearing a diaper[end if] [if the wanktime of the player > 0]so soon after a previous orgasm [end if]has a [italic type]chance[roman type] of the girls running the game punishing you for it. Do you want to go ahead and risk it? ";
		unless the player is in agreement, say "You decide against it." instead;
	if the buttskill of the player is 1 or asswank is 1:[if asswank is already 1, it means the player can only masturbate anally.]
		if asswank is 1 and auto is 0:[If its automatic masturbation, we won't notify the player.]
	...
	otherwise if auto is 0 and the size of penis > 0 and penis is not erect-at-will and penis is not penis-erect:
...
Carry out masturbating:
	...
	otherwise:
		...
		if auto is 0 and the wanktime of the player > 0 and the number of on-stage chastity cages is 0 and the number of worn diapers is 0:[Put the player in chastity if necessary]
...
To say InstantMasturbationFlav of (O - an object) with (M - a thing) in (C - an object):[We're only doing an introduction here, the rest is handled by the climaxmasturbationflav function; sex addiction here should be raw sex addiction]
	...
	if the player is male:
		...
		if R is 1:
			say "You [if O is vibe-wand]turn on the [ShortDesc of O] and grind your [ShortDesc of penis] against it[otherwise if C is clothing]stroke your [ShortDesc of penis] through your [ShortDesc of C][otherwise]eagerly begin to stroke your [ShortDesc of penis][end if], [if M is a live thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasising about [PlayerFantasy][end if]. [if A < 5]It's almost embarrassing how much it turns you on[otherwise if A < 10]It's a little surprising how much it turns you on[otherwise]It makes you uncontrollably turned on[end if], and as the pleasure rapidly builds upon itself, you realise you're already about to cum.";
		otherwise if R is 2:
			say "You [if O is vibe-wand]turn on the [ShortDesc of O] and[end if] begin to rub your [ShortDesc of penis] [if C is clothing]through your [ShortDesc of C][end if], closing your eyes as you [if M is a live thing]focus on [NameDesc of M][otherwise]fantasise about [PlayerFantasy][end if]. Nothing more than a moment passes, [if A < 5]and you are immediately ashamed to[otherwise if A < 10]and you tell yourself you should be ashamed to[otherwise]and you feel your muscles tightening in anticipation as you[end if] realise that you're already going to cum.";
		otherwise if R is 3:
	...
	otherwise:
		...
		otherwise if O is vibe-wand:
			say "[one of]You turn on the [printed name of O] and grind yourself against it, [if M is a live thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasising about [PlayerFantasy][end if]. [if A < 5]It's a little embarrassing how much it turns you on[otherwise if A < 10]It gets you even more turned on[otherwise]It makes you uncontrollably turned on[end if], and as the heat of arousal lights up your body, you realise you're already going to cum.[or]You switch on the [printed name of O] and hold it against your clit, closing your eyes as you [if M is a live thing]focus on [NameDesc of M][otherwise]fantasise about [PlayerFantasy][end if]. Already, the rapidly intensifying pleasure seems to head toward a peak, and you realise [if A < 5]with shame [otherwise if A < 10] [otherwise]with anticipation [end if]that you're about to cum.[or]You turn on the [printed name of O], holding the vibrating end against your clit. [if A < 5 and M is a live thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a live thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realise you're already going to cum.[at random]";
		otherwise if C is clothing:
			say "[one of]You eagerly begin to rub your clit through your [ShortDesc of C], [if M is a live thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasising about [PlayerFantasy][end if]. [if A < 5]It's a little embarrassing how much it turns you on[otherwise if A < 10]It gets you even more turned on[otherwise]It makes you uncontrollably turned on[end if], and as much as the sensation is dampened by the [clothing-material of C], you realise you're already going to cum.[or]You begin to rub yourself through your [ShortDesc of C], closing your eyes as you [if M is a live thing]focus on [NameDesc of M][otherwise]fantasise about [PlayerFantasy][end if]. Even through the fabric, the pleasure rapidly intensifies, and you realise [if A < 5]with shame [otherwise if A < 10] [otherwise]with anticipation [end if]that you're about to cum.[or]You touch yourself through the [ShortDesc of C], teasing your clit through the fabric. [if A < 5 and M is a live thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a live thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realise you're already going to cum.[at random]";
		otherwise:
...
To say StartMasturbationFlav of (O - an object) with (M - a thing) in (C - an object):
	...
	otherwise if auto < 2:
		if O is insertable object:
			if R is 1:
				say "You [if the player is not horny]sigh softly[otherwise if the player is horny]moan quietly[otherwise]shudder with pleasure[end if] as you slide the [ShortDesc of O] into your [vagina] and begin to fuck yourself.";
			otherwise if R is 2:
		...
		otherwise if O is vibe-wand:
			if R is 1:
				say "You [if the player is not horny]sigh softly[otherwise if the player is horny]moan quietly[otherwise]shudder with pleasure[end if] as you switch on the [ShortDesc of O] and hold the bigger end just under your clit.";
			otherwise if R is 2:
		...
		otherwise:
			if R is 1:
				say "You sink your fingers into your [vagina], [if the player is not horny]sighing softly[otherwise if the player is horny]moaning quietly[otherwise]shuddering with pleasure[end if] as you gently push them in and out.";
			otherwise if R is 2:
...
To say StartAnalMasturbationFlav of (O - an object) with (M - a thing) in (C - an object):
	...
	if O is insertable object:
		...
		otherwise if R is 2:
			say "You reach between your legs, [if the player is not horny]sighing softly[otherwise if the player is horny]moaning quietly[otherwise]shuddering with pleasure[end if] as you slowly begin to fuck yourself with the [printed name of O].";
		otherwise:
			say "You [if the player is not horny]sigh softly[otherwise if the player is horny]moan quietly[otherwise]shudder with pleasure[end if] as you reach between your legs and sink the [printed name of O] into your [asshole], and slowly begin to fuck yourself.";
	otherwise:
		if R is 1:
			say "You reach between your legs and slip your fingers into your [asshole]. You [if the player is not horny]sigh softly[otherwise if the player is horny]moan quietly[otherwise]shudder with pleasure[end if] as you begin to gently push them in and out.";
		otherwise if R is 2:
			say "You reach between your legs, [if the player is not horny]sighing softly[otherwise if the player is horny]moaning quietly[otherwise]shuddering with pleasure[end if] as your fingers gingerly enter your [asshole] and slowly begin to push in and out.";
		otherwise:
