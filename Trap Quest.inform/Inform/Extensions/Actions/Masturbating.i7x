...
To say PlayerFantasy:
	...
	otherwise if the bimbo of the player < 15:[degradation from strangers]
		say "[one of]getting fucked until [if the player is possessing a vagina and pregnancy fetish is 1]you're pregnant[otherwise]you can't walk[end if][or]being pinned down and fucked by muscular [men of shopkeeper] with big hard [DickDesc of shopkeeper]s[or]being blindfolded, bound, and stuffed full of dildos[or][if the player is gendered female]arriving at an important meeting and immediately getting slammed on the table and fucked by the majority shareholder[otherwise]making money on porno sets as a cock warmer between scenes[end if][or][if bukkake fetish is 1 and a random number between 1 and 2 is 1]the feeling of thick, warm [semen] rolling down your face[otherwise if watersports fetish is 1 and a random number between 1 and 2 is 1]the feeling of warm, frothy [urine] streaming into your mouth[otherwise if the raw semen taste addiction of the player > 10]the feeling of thick, warm [semen] spurting into your mouth and rolling down your throat[otherwise]the feeling of a warm, hard [DickDesc of shopkeeper]s in your mouth[end if][in random order]";
	otherwise:[even more degradation from strangers]
		say "[one of]getting gangbanged in the alleyway behind your favourite bar[or][if the player is gendered female]using all that time you wasted in business school to further your career as porn actress[otherwise]becoming a pornographic actress[end if][or]waking up bound, naked, and surrounded by a gang of complete strangers with big hard [DickDesc of shopkeeper]s[or]giving a wet, sloppy blowjob to every [man of shopkeeper] that passes you in the street[or]play-testing even more [i]fun[/i] games for Nintendolls[in random order]".
...
Check masturbating:
	...
	if auto is 0:
		...
		if the player is pee protected and the player is possessing a penis:
			say "You [if wanktype is ASS-WANK]are still wearing your[otherwise]will get yourself off through your[end if] [ShortDesc of random worn bottom level pee protection clothing] which will cause you to ejaculate into it, is that what you want? ";
			if the player is not bimbo consenting, say "Then you better [bold type]remove[roman type] or [bold type]displace[roman type] the [random worn bottom level pee protection clothing] first." instead;
	...
	if (wanktype is COCK-WANK or wanktype is HERM-WANK) and penis is not erect-at-will and penis is not penis-erect:
		if wanktype is COCK-WANK:
			say "Your [ShortDesc of penis] remains completely soft, so you give up on doing anything with it.";
		...
		otherwise:[must be a herm situation]
			if auto is 0:
				say "Your [ShortDesc of penis] remains completely soft. Do you want to continue without using your penis?";
				if the player is bimbo consenting:
...
To say InstantMasturbationFlav of (O - an object) with (M - a thing) in (C - an object):[We're only doing an introduction here, the rest is handled by the climaxmasturbationflav function; sex addiction here should be raw sex addiction]
	...
	otherwise if C is clothing:[If C is clothing, we know M isn't in the way. Also, O isn't a thing beyond this point]
		say "[one of]You eagerly begin to grab and rub [if wanktype is HERM-WANK]both sets of genitals[otherwise if wanktype is COCK-WANK]your [ShortDesc of penis][otherwise]your clit[end if] through your [ShortDesc of C], [if M is a live thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasising about [PlayerFantasy][end if]. [if A < 5]It's a little embarrassing how much it turns you on[otherwise if A < 10]It gets you even more turned on[otherwise]It makes you uncontrollably turned on[end if], and as much as the sensation is dampened by the [clothing-material of C], you realise you're already going to cum.[or]You begin to rub yourself through your [ShortDesc of C], closing your eyes as you [if M is a live thing]focus on [NameDesc of M][otherwise]fantasise about [PlayerFantasy][end if]. Even through the fabric, the pleasure rapidly intensifies, and you realise [if A < 5]with shame [otherwise if A < 10] [otherwise]with anticipation [end if]that you're about to cum.[or]You touch yourself through the [ShortDesc of C], [if wanktype is HERM-WANK]teasing your clit and groping your shaft[otherwise if wanktype is COCK-WANK]groping your [ShortDesc of penis][otherwise]teasing your clit[end if] through the fabric. [if A < 5 and M is a live thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a live thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realise you're already going to cum.[in random order]";
	otherwise:
		if wanktype is not VAG-WANK:[must be penis wank or herm wank]
			say "[one of]You eagerly begin to stroke your [ShortDesc of penis][if wanktype is HERM-WANK] and play with your [vagina][end if], [if M is a live thing]focusing all your thoughts on [NameDesc of M][otherwise]fantasising about [PlayerFantasy][end if]. [if A < 5]It's almost embarrassing how much it turns you on[otherwise if A < 10]It's a little surprising how much it turns you on[otherwise]It makes you uncontrollably turned on[end if], and as the pleasure rapidly builds upon itself, you realise you're already about to cum.[or]You begin to pump your [ShortDesc of penis][if wanktype is HERM-WANK]and tickle your clit[end if], closing your eyes as you [if M is a live thing]focus on [NameDesc of M][otherwise]fantasise about [PlayerFantasy][end if]. Nothing more than a moment passes, [if A < 5]and you are immediately ashamed to[otherwise if A < 10]and you tell yourself you should be ashamed to[otherwise]and you feel your muscles tightening in anticipation as you[end if] realise that you're already going to cum.[or]You [if wanktype is HERM-WANK]tease your clit and [end if]eagerly begin to stroke your [if penis is penis-erect]hard[otherwise]hardening[end if] [ShortDesc of penis]. [if A < 5 and M is a live thing]It feels wrong, but you focus on [NameDesc of M][otherwise if M is a live thing]You focus as much as you can on [NameDesc of M][otherwise]A fantasy of [PlayerFantasy] enters your thoughts[end if], and just like that, you realise you're already going to cum.[in random order]";
		otherwise:
		...
		if wanktype is HERM-WANK:
			...
			otherwise:
				if penis is penis-erect:
					say "You slip a finger into your pussy, [one of]grab your erect[or]wrap your hand around your hard[or]grasp your stiff[in random order] [ShortDesc of penis] and begin to stroke.";
				otherwise:
					say "[one of]You sink your fingers into your [vagina], [if the player is not horny]sighing softly[otherwise if the player is horny]moaning quietly[otherwise]shuddering with pleasure[end if] as your [ShortDesc of penis] slowly stirs to life.[or]You reach between your legs, parting your [if the semen coating of thighs > 0]slimy, [semen] glazed labia lips[otherwise]delicate labia lips[end if] with your fingers, allowing your [ShortDesc of penis] to grow stiff and hard as your fingers sink into your [vagina].[or]You allow your fingers to dip into your [vagina], your [ShortDesc of penis] growing harder and harder by the second.[in random order]";
