...
Carry out mopping face:
	...
	let D be 0;
	say "You begin [one of]to lick up[or]slurping up[or]licking up[at random] the [if C > 0 and C > B + A][semen] clinging to the floor[otherwise if A > 0 and A > C + B][urine] on the floor[otherwise if B > 0 and B > C + A]milk on the floor[otherwise]fluids on the floor[end if], [if the humiliation of the player < HUMILIATION-SHAMELESS][one of]shuddering at the new low this game has forced you to.[or]once again finding yourself scraping the bottom of the barrel for humiliating acts.[stopping][otherwise][one of]shivering with excitement at your brand new low.[or]once again finding yourself at the peak of degrading acts.[stopping][end if]";
	if C > 0:
		StomachSemenUp 1;
	...
	if class of the player is catgirl:
		StomachUp 1;
		if the humiliation of the player < HUMILIATION-BROKEN, humiliate SEVERE-HUMILIATION - (MODERATE-HUMILIATION * (the thirst of the player - 1));
	otherwise:
...
Carry out mopping pink spraybottle:[TODO:  breasts largeness 10+ will touch the ground and become soaked.]
	...
	MagicPowerUp 1;
	say "You get to work cleaning up the mess with your [printed name of the noun]. [if total puddle <= E * 3]It feels like [noun] is working together with you to clean up the mess, and you get the room clean in a short time![otherwise if the noun is not cursed]It feels like the washcloth is working together with you to clean up the mess, and although [bold type]you're not finished[roman type] cleaning up the mess, you know you're making record time![otherwise]It feels like the washcloth is actively working against you to clean up the mess, and even though you're working hard, you don't make a lot of progress cleaning up the mess.[end if]";
