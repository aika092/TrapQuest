...
To compute turn (N - a number) of (M - a monster):
	...
	otherwise if M is awake:[Monsters can't do anything if they are sleeping]
		...
		if M is penetrating a body part or M is grabbing the player, now M is in the location of the player; [Just in case there's a glitch where the player remains stuck but can't interact with the monster because somehow it's in a different room]
		if M is in the location of the player, check immobility; [It's important that NPCs are aware whether the player is already immobile or not when deciding how to interact with the player]
		if N is not 2:
...
To compute action (N - a number) of (M - a monster):
	...
	if M is in the location of the player and M is undefeated and M is not penetrating a body part and M is not grabbing the player and M is friendly and (M is interested or N is 1), check disapproval of M;
	if M is interested:
	...
	otherwise if M is not distracted and M is not caged and M is not guarding and (M is undefeated or M is not motionless-when-defeated):
		if the boredom of M is 0 and M is unleashed and (cowbell is clanking or the player is glued seductively), check seeking N of M;
